#!/bin/sh
# Capture driver for the native operator ceremony, protocol revision 8.
#
# Usage: scripts/live-native-operator-capture.sh <new-capture-directory>
#
...
if [ "$#" -ne 1 ]; then
  die "usage: live-native-operator-capture.sh <new-capture-directory>"
fi
...
operator_test=the_live_transfer_candidate_runs_against_a_real_target
expected_rust_test="${#operator_test} $(printf '%s' "$operator_test" | od -An -tx1 | tr -d ' \n')"
...
phase_start report
cargo_argv="$capture_cargo test -p tripod-vectors --test maturity_operator -- --ignored --test-threads=1"
cargo_argv_hex=$(printf '%s' "$cargo_argv" | od -An -tx1 | tr -d ' \n')

{
  printf 'run-report-schema native-operator-run-report 1\n'
  printf 'capture-format-schema native-operator-capture 1\n'
  printf 'suite-commit %s\n' "$suite_commit"
  printf 'suite-tree %s\n' "$suite_tree"
  printf 'suite-clean yes\n'
  printf 'rust-test-target maturity_operator\n'
