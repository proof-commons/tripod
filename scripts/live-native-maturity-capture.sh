#!/bin/sh
# Capture driver for the native maturity announcement ceremony, protocol revision 8.
#
# Usage: scripts/live-native-maturity-capture.sh <new-capture-directory>
#
...
#   cargo test -p tripod-vectors --test maturity_native -- --ignored --test-threads=1
...
if [ "$#" -ne 1 ]; then
  die "usage: live-native-maturity-capture.sh <new-capture-directory>"
fi
...
maturity_test=the_maturity_announcement_runs_against_a_real_target
...
phase_start report
cargo_argv="$capture_cargo test -p tripod-vectors --test maturity_native -- --ignored --test-threads=1"
cargo_argv_hex=$(printf '%s' "$cargo_argv" | od -An -tx1 | tr -d ' \n')

{
  printf 'run-report-schema native-maturity-run-report 1\n'
  printf 'capture-format-schema native-maturity-capture 1\n'
  printf 'suite-commit %s\n' "$suite_commit"
  printf 'suite-tree %s\n' "$suite_tree"
  printf 'suite-clean yes\n'
  printf 'rust-test-target maturity_native\n'
