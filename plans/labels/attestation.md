...
| Task | ``task`` | A tracked unit of backlog work: kept, statused, cited long after writing, a records-family genre. The registry's Task rows are ``exer``, work set for a reader, and ``job``, a code asset that runs; both misdescribe a backlog item, and ``task`` is assigned to nothing | ``task:phase2:pilots`` | `plans/backlog.md:1231` | 12 |
