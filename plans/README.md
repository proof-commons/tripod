...
excluding the archived documents and generated registers below, is the core class. Its hard cap is 1536 KiB (1.5 MiB), raised on 2026-09-12 when the tracked tree measured 1207999 bytes against the 1208320-byte cap — 321 bytes of headroom — while Phase 6's first wave was still recording; it is a round 1.5 MiB expressed in whole KiB. Its soft target is 1200 KiB, raised on 2026-09-19 from the 1038 KiB the 2026-09-12 raise derived as the old 798 KiB scaled by the same ratio, `798 × 1536 / 1180 = 1038.7525 KiB`, and rounded down; the new figure is set from measured headroom rather than from a ratio, because the hard cap is unchanged and the tracked tree measured 883525 bytes once the closed Phase-6 task rows moved into history — a target that tree would re-approach inside one phase warns recurrently instead of signalling. The 2026-09-12 raise is paired with moving the exited Phase-3, Phase-4, and Phase-5 closed rows into history, so its room is for the remaining Phase-6 waves rather than duplicated closed records. The earlier raises are recorded with their derivations at the constants. That prose is maintained, so unchecked growth there is duplication rather than content, and the budget keeps one fact to one owner; a raise moves where duplication starts costing, and removes nothing.
