...
| (`inf:labels:local-mint`) | implemented | Bare spans mint into the owner's own registry in `harvest_markdown_owner` and `harvest_label`. |
| (`inf:labels:local-citation`) | implemented | `mint_nodes` in `build_label_graph` is keyed on the owner-and-label pair, so a local citation resolves across that owner's files and nowhere else. |
| (`inf:labels:import`) | implemented | `import` in `repository.rs`; unknown prefixes raise `UnknownOwner`, and `diagnose_bracket_free_owner_token` fails a cross-owner token written without brackets. |
...
| (`mthm:labels:order-independence`), (`mthm:labels:no-self-support`) | implemented | The first follows from the staging above. For the second, index lines are excluded from the body scan and only body tokens enter the anchor set; `validate_attestation_anchor_pin` recomputes the pin over body names alone. |
| (`mthm:labels:presentation-invariance`) | undetermined | A meta-theorem about migrations, not a checkable property; no code asserts or could assert it. |
...
### Postcondition checklist · `sec:gapcensus:calculus-postcondition`

Each item of (`postc:labels:implementation`) against the current
checker.

| Postcondition item | Status |
|---|---|
...
| every local and imported citation resolves to one mint | holds |
| bracket-free cross-owner tokens and self-qualified imports fail | holds |
...
The one real gap here is the narrowness of the synthetic-citation and
anchor-harvest mechanisms: each is a correct instance of its rule,
neither is the rule.
...
Forty-three kinds in current use are assigned identically by the
registry, among them `sec`, `rule`, `rem`, `inv`, `def`, `dec`, `lem`,
`inf`, `judg`, `cav`, `mthm`, `ansatz`, `conv`, `thm`, `case`, `tab`,
`gate`, `crit`, `red`, `moral` and `postc`. The rest part:

| Repository kind | Registry position | Disposition |
|---|---|---|
| `subsec` | Subsection is `sec`; the sub- prefix is a presentation device | divergent, and it is also a hard-listed realization kind in `label.rs` |
| `tbl` | Table is `tab` | divergent; both spellings are in use, which the one-kind invariant forbids |
| `lst` | Listing is `listing` | divergent |
| `protocol` | Protocol is `proto` | divergent; the long form is a hard-listed model kind in `rust_source.rs` |
| `post` | Postcondition is `postc` | divergent; both spellings are in use |
...
| `ins` | Insight is `intuit` | divergent if the repository token abbreviates Insight |
| `err` | Erratum is `errat`; an error-vocabulary genre is uncatalogued | absent from the registry |
| `trap`, `pin`, `req`, `obl`, `candidate`, `milestone`, `phase`, `pkg`, `op`, `leaf`, `ref`, `test`, `branch` | no row | absent from the registry |
...
environment head is in use. Adoption would be a rename of at least five
tokens plus a registry extension of about thirteen, and not one the
checker can drive: the realization and model kind lists are compiled
into `label.rs` and `rust_source.rs`, so any rename is a code change
and a register regeneration in one commit.

One defect internal to the draft: its classification judgment
(`judg:kinds:classification`) says eight Conventions form the complete
graph, while the document carries eleven.
...
| Clause | Disposition | Evidence |
|---|---|---|
| (`rule:identity:validation-order`), (`myth:identity:hashes-validate`) | implemented | ADR-016 states it, and the producers in `packages/architecture/src/canonical.rs` and `deployment.rs` hash validated exports only. |
| (`tab:identity:mechanisms`), (`warn:identity:non-claims`), (`cond:identity:admission-record`) | implemented | The mechanism table is reproduced as ADR-016's; every register entry carries subject, owner, producer, consumer, decision, assurance, stale condition, recipe, migration, non-claims and status. |
...
| (`case:identity:none`) | divergent | The register's dormant and provisional statuses record identities awaiting consumers, but no entry records a deliberate refusal to hash as its own documented outcome. |
...
| (`rule:identity:containment`) | implemented | Publication-only provenance identities are contained by the register's status vocabulary. |
| (`gate:identity:implementation`) | divergent | Every item ADR-016 already gates is met; the items about recipe records, the benefit walk and the release surfaces are not. |
...
| (`sig:interchange:theory-assignment`), (`inv:interchange:permanence`), (`inv:interchange:patch`), (`inv:interchange:minor`), (`law:interchange:major-boundary`) | absent | An owned registry with immutable assignments, a machine check of content-class inclusion across minors, and a version-bump gate wired to it. |
| (`def:interchange:acceptance`), (`mthm:interchange:bounded-determination`) | absent | Envelope-first dispatch with whole-document rejection; the bound then follows from the encoder. |
...
3. The kind registry classifies Table as `tab` and Task as `exer`,
   while the repository uses `tbl` throughout and `task` for work
   items. Adoption implies a corpus-wide rename touching compiled kind
   lists; the alternative is to record the repository's tokens as a
   local extension. Thirteen further kinds the repository relies on,
   `trap` and `req` and `obl` among them, have no row at all.
4. The repository uses both `tab` and `tbl`, and both `post` and
   `postc`, which the one-kind invariant forbids independently of
   adoption. One spelling of each must go.
