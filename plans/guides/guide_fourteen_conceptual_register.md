...
| `G14C-07` | Semantic and complete-environment backend determinism | [A-F6](../reviews/guide_fourteen_conceptual_review_a.md) | OBSERVATION | §§7.5, 11.2–11.4, 12.3–12.8, 15.4, 16.1 | 2, 7 | Wave 2 states that equal canonical world plus request and bound protocol context determine one semantic successor, while Wave 7 states that a bound semantic execution plus the complete deployment, branch, funding, sponsorship, fee, and signer-public environment determines exact candidate bytes. Property tests prove equality within each complete input tuple and demonstrate that distinct admitted sponsor environments need not share transaction bytes. | CLOSED — Equality within the complete input tuple is a test rather than a restatement: two independently linked bundles are carried through view, request, construction, finalization and signing and reach equal candidate bytes, equal protected bytes and equal witness stacks, with one admitted cycle moved as the control that keeps the equality from being a statement about a constant, while the second demonstration over distinct admitted sponsor environments belongs to the sponsored arm ruling 58 defers and is recorded as the half that did not close (`0.6.210-dev`). |
| `G14C-08` | Authenticated cuts form a feedback-edge proof | [B-1](../reviews/guide_fourteen_conceptual_review_b.md) | CONCEPT-BLOCKER | §§9.7, 11.2, 21–22; Wave 6 | 6 | Wave 6 classifies and validates every runtime cut, removes all validated cut edges, requires the residual dependency graph to be acyclic, derives a canonical topological order, and includes a negative SCC with two edge-disjoint cycles where one authenticated cut leaves one cycle and must refuse. | CLOSED — Three validated cuts are removed and the residual graph is acyclic with a canonical dependency order placing every referent before its referrer, while a component carrying two edge-disjoint cycles under one authenticated cut is refused with the surviving cycle and the single removed cut named (`0.6.193-dev`). |
| `G14C-09` | Binding-time types make the recursive symbol safe | [B-2](../reviews/guide_fourteen_conceptual_review_b.md) | CONCEPT-BLOCKER | §§1.7–1.8, 9.5–9.7, 11.1–11.3, 12.6 | 6 | Wave 6 gives every reference a closed binding-time type, represents `StateStaticSubtree` only as a spend-time introspected or witnessed-then-authenticated strategy, proves that its root value is never serialized as a literal beneath itself, and tests the exact introspection role, reconstruction equation, and equality check while refusing ordinary link-time relocation of that symbol. | CLOSED — Five closed binding-time forms fix which references resolve a cycle and what evidence each cut must present, a link-time static root beneath its own program is refused as such, ordinary relocation of that symbol is refused for carrying no pushable item, and the witnessed root's authentication is exact against the internal key and the authenticated metadata (`0.6.189-dev`, `0.6.193-dev`). |
| `G14C-10` | Canonical linker normal form and permutation invariance | [B-5](../reviews/guide_fourteen_conceptual_review_b.md) | DESIGN-RISK | §§7.5, 11.2–11.4, 21; Wave 6 | 6 | Wave 6 defines stable keys for definitions, reference edges, SCC members and identities, condensation-DAG ties, relocation sites, leaves, equal-cost tree candidates, and serialization; rebuilds substitutions simultaneously from pristine structured input and reparses the result; and proves byte equality under permutations of definitions, references, relocations, leaves, and equal-cost ties. | CLOSED — Definition, edge, component, relocation-site and leaf identities are stable under permuted input, one rebuild carrying every value together is reparsed and reaches the program the per-key rebuilds reach in either order, and the equal-cost-tie clause has no subject while the ruled static tree is the singleton whose cost is zero and whose declarations the constructor refuses to vary, which a generation admitting equal-cost candidates would re-open for that generation (`0.6.191-dev`, `0.6.193-dev`, `0.6.194-dev`). |
...
Wave 6 closed its three owned rows on ruling 50's criteria and adopted no candidate: the residual-DAG proof with its edge-disjoint negative closes `G14C-08`, the closed binding-time forms with the refusal of a literal static root beneath itself and exact witnessed-root authentication close `G14C-09`, and the stable identities with one reparsed rebuild under permuted input close `G14C-10`, whose equal-cost-tie clause has no subject while the ruled static tree is the singleton, while `R-5 RECOVERY` retains its Wave-10 trigger (`0.6.189-dev`, `0.6.191-dev`, `0.6.193-dev`, `0.6.194-dev`).

Wave 7 owns `G14C-07` and the host-policy refusal clause of `G14C-03`, and opens with no candidate adoption: ruling 56 at (`rule:phase6:wave7-rulings`) fixes the refusal that discharges the `G14C-03` clause, an exhausted successor search reaching the caller as the linker's own exhaustion refusal wrapped in one transaction variant rather than flattened, which is a host-policy refusal making no claim about the target's validity; and ruling 54 fixes the signer under which a submit-ready candidate is reachable at all, the third published test signer handle whose x-only key a third maturity deployment commits, because both deployments in the tree commit public material nobody holds a scalar for. `G14C-07` closes on equal complete input tuples giving equal candidate bytes, which row `T11-074` carries as a test over the node-free submit-ready candidate, while the row's second demonstration — that distinct admitted sponsor environments need not share transaction bytes — belongs to the sponsored arm ruling 58 defers, so the closure record states which half closed. `R-5 RECOVERY` remains the register's sole CANDIDATE, its trigger belonging to Wave 10 and unchanged by this record.

Wave 7 closed the row it owns and the clause it shares, and adopted no candidate: `G14C-07` closes on equal complete input tuples giving equal candidate bytes, carried as a test over two independent builds of one tuple with a one-term control beside it, while the row's second demonstration over distinct admitted sponsor environments belongs to the sponsored arm ruling 58 defers and is recorded as the half that did not close; and the host-policy refusal clause of `G14C-03` is discharged by an exhausted successor search arriving as the linker's own exhaustion refusal wrapped in one transaction variant, which leaves that row OPEN for Wave 12's bounded corpus alone. `R-5 RECOVERY` remains the register's sole CANDIDATE, its trigger belonging to Wave 10 and unchanged by this record (`0.6.207-dev`, `0.6.210-dev`).

Wave 8 owns `G14C-12` with Wave 10 and shares `G14C-02` with Waves 2, 9, 10 and 13, and opens with no candidate adoption: ruling 66 at (`rule:phase6:wave8-rulings`) fixes this wave's share of `G14C-12` as the closed three-class observation vocabulary, the class-3 poison marker placed beside the STATE safety standings so that no ordinary standing can accept it by type, the class-1 and class-2 and no-class-3 quantifier on every Wave-8 guarantee, and one class-3 test over a model-falsifying byte observation with no abstraction preimage, leaving the typed class-2 responses and the three environment tests — competing spend, same-key instance, reorganization-surfaced realization — to Wave 10, whose branch context this wave has no way to observe; and ruling 68 fixes the clause the safety report witnesses for `G14C-02`, soundness in the form that every refused concrete step corresponds to no allowed semantic step at its declared boundary, while the forward-simulation clause waits on an accepted step and on Wave 9's constructor projections. Ruling 64 fixes that the native run is this wave's evidence rather than its exit and that the seventh conjunct of the Phase-6 gate, one sponsorless positive control accepted by a real target, is not met by this wave, so neither owned row closes on a target acceptance. `R-5 RECOVERY` remains the register's sole CANDIDATE, its trigger belonging to Wave 10 and unchanged by this record.

The register contains eight OPEN rows and seven CLOSED rows and 26 wave-ownership assignments. R-2 additionally feeds `G14C-11` constraints into Waves 7 and 9, and R-TAXONOMY shares `G14C-12` vocabulary with Waves 7 and 9 without changing the primary ownership counts above.
