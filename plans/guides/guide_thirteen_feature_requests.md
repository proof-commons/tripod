...
**RESOLVED as a predicate-versus-registry conflict, under T5-041, and the row still has not moved.** The third option was taken. The registry now admits a single-output fully-solved balancing form, declared by a sole-balancing output role rather than inferred from a count, with the two-output case bit-for-bit as it was and the floor still refusing a lone output that does not declare the form. The condition this erratum attached to that option was met by both of its admitted answers at once: the registry refuses a solved zero blinder outright, a refusal it already carried and which the form makes load-bearing, and the shape that ran was built from inputs that cannot cancel. A target accepted the strict one-to-one.

So the conflict this erratum records is over: the merge predicate — inputs at least two and outputs exactly one — is satisfiable as the guide states it, and the two rules that were mutually unsatisfiable no longer are. A future guide needs neither of the first two options, which existed only to work around a floor that has been removed.

**What has NOT happened is the private-merge row moving, and the reason is a new one.** The registry admits a merge; this workspace's confidential ceremony cannot yet offer one any coins to merge. It funds a single predecessor from an explicit input, so that predecessor's two output blinders are ordered additive inverses, and a merge consuming both halves of an inverse pair presents a zero input blinder sum — forcing the lone output's blinder to zero, which is exactly the degeneracy this erratum's own condition names. The registry refuses it, correctly. The wall the merge meets has moved from the output count to the blinders, which is to say from an accident of a construction model to the confidentiality property that actually distinguishes a merge worth building; the row stays unmoved and typed, on that new ground rather than on this erratum's. The removal path — a chained precursor whose outputs do not cancel — is filed in the shape-possibility register at (`rule:shapes:canceling-predecessor`).

**THE ROW HAS MOVED, under T5-045, and the erratum's arc is complete.** A target accepted a private merge: two receipts consumed, one output created, 5156 bytes, one range proof of 4174, the mined bytes equal to the submitted ones and the read-back witness verifying against an independently recomputed message. The condition this erratum attached to the third option is met on its own terms rather than by the registry's refusal standing in for it: the merge's forced blinder is NONZERO, and the ceremony observes that rather than arguing it, summing the coins it actually consumes and comparing the result with the blinder the registry solved. What made the coins available is a second predecessor of three outputs, funded exactly as the dual-parity one is and differing in its output count alone -- three blinders summing to zero cancel in no pair, any two of them summing to the negation of the third, and no blinder is ever zero, a derived one being searched until it is nonzero and a solved one refused when it is not. One divergence from the filed path is reported and is not a defect: the path named a CHAINED precursor submission and what was built is a wider funding stage, because a funding response carries decoded coins with their openings while a submission response carries none, so chaining would have meant building a coin-return path the ceremony does not have. The arithmetic is the arithmetic the path named. Both walls this erratum recorded are now behind the shape, and neither record of them is deleted: an erratum whose history is edited away cannot show that a wall stood there, and a wall nobody remembers is one that gets rebuilt.
...
**Confidential-funding guide §10.5, the mandatory chain makes step 7 unreachable rather than merely deferred.** The restart order (`task:guide-ctf-exec:restart-order`) opens by stating that the order is mandatory and that each step's entry is the previous step's observed result. Steps 6 and 7 then read: sponsor cases only after their independent signer dependency closes by observation, and disclosure-minimality pairs last, only after both sides of each pair accept. Read together and strictly, those three sentences are stronger than the guide's text appears to intend. Step 6 is the one step whose entry is not an observed result of the previous step at all but a named external dependency, and the guide does not close that dependency anywhere. So while the dependency stands, step 6 produces no observed result, and the chain rule then denies step 7 its entry — not until later, but permanently, since nothing in the guide can ever supply what step 6 is waiting for. Step 7's own clause asks only that both sides of each pair accept, which is a condition about the pairs and says nothing about sponsors; the unreachability comes entirely from the chain rule above it and not from step 7's stated gate. The strict reading was taken rather than worked around: no pairs were run and no minimality row moved through three waves, and the unreachability was held structurally by a test asserting the ledger refuses the pairs after the stop.

The dependency has since closed, and closing it is what makes the defect legible rather than what removes it. The signer dependency turned out to be a first-party defect in this workspace and not a property of the target: the demonstration deployment's fee-role program digest was a fixture constant no program hashes to, so every sponsored control it built was refused at its own §10.7 fee-role check. Threading that digest to the value the target computes for the empty fee program construction writes produced an observed acceptance, and step 6 now has an observed result. But the ordering rule that held step 7 hostage is unchanged, and it held it hostage to a symbol bug in a repository the guide does not own — which is precisely the case a mandatory chain should not have been able to reach. The scope of this erratum is the chain rule's strength only. The mandatory order is otherwise correct and is not relaxed: steps 1 through 5 genuinely do depend on each other's observed results, and a negative case run before step 1 is not evidence about its own mutation, exactly as §10.5's closing sentence says. A future guide should qualify the chain rule so that a step typed-stopped on a NAMED EXTERNAL DEPENDENCY does not consume the entry of the steps after it, and should admit non-sponsor disclosure-minimality pairs on step 5's observed result — the pairs whose two sides involve no sponsor have no dependency on step 6's subject matter, and making them wait on it buys the evidence nothing.

**Confidential-funding guide §15.6, `zero-valued-ordinary-sponsor-member` is listed as a fault and the sources rule it an acceptance.** The row predicts a script-path rejection. No layer produces one, and the guide series had already ruled the question twice before the row was transcribed. Guide 8 §22.6 corrects this exact contradiction by name — "zero-valued ordinary sponsor member with exact role structure: semantic acceptance" — and instructs in its next line that no generic domain-failure vector for zero-valued ordinary sponsor output be preserved; the §15.6 row is a preserved generic domain-failure vector. Guide 12 gives the layered reading the row flattens: the semantic relation may accept exact role structure, the first-party explicit builder omits known zero change as construction policy, and a deployment may reject it as nonstandard. The realization's sponsor-opacity rule is why: sponsor-value opacity removes ordinary sponsor L-BTC amounts from the protocol read-set entirely, so a sponsor-role `PLAIN_LBTC` member carries `SponsorOpaque` and there is no amount for any relation to compare with zero (`packages/realization/src/evaluate.rs`, the `ObjectId::PlainLbtc` arm, whose own comment states that a zero-valued sponsor member is an ordinary member like any other). The relation's deciding tests drive it twice and both accept — `zero_sponsor_sidecar_is_accepted` and `mixed_zero_and_positive_sponsor_members_are_accepted`, in `packages/realization/src/tests/live_transfer_tests.rs` — while the two adjacent tests that DO fail, `zero_sponsor_input_without_signature_fails_isolation` and `unclaimed_zero_sponsor_member_fails_isolation`, fail on exact membership rather than on value, which is a different row's fault. The first-party builder does not refuse a zero residual either; it declines to emit one (`packages/transaction/src/construct.rs`, `is_known_zero`, whose site records that the target refuses a spendable zero-valued output so emitting one would produce a transaction consensus rejects), so no candidate carrying this row's fault can be built here at all.

The request is that §15.6 stop listing this row among its faults, per Guide 8 §22.6's own instruction. This workspace has not edited the row's declaration: the matrix's tables are polarity-homogeneous by construction, so a fault-table row cannot be made positive without moving tables, and which table §15.6 lists is the guide's to say. The row's STANDING is corrected instead, to a first-party fact citing the deciding test, and the declaration is annotated at its own site so the disagreement is legible rather than silent.

**Confidential-funding guide §15.5, `duplicated-destination` is listed as a fault and the request type reads it as an ordinary split.** The row predicts a script-path rejection for two identical destinations. Its own sibling `duplicated-source` is a real fault — §12.1 refuses a repeated receipt outpoint before sorting rather than collapsing it, because a selection built by insertion would silently consume one coin for a caller who asked for two — and the contrast is what makes the mis-typing visible. §12.2 keeps the DESTINATION census a multiset for the opposite reason, stated in `packages/transaction/src/live_request.rs` at `destination_multiset`: two destinations of the same owner and value are two receipts, and a set would report them as one. The deciding test builds exactly that pair and records the verdict in its own name and its own words — `two_destinations_of_one_owner_and_one_value_are_two_receipts`, in `packages/transaction/src/tests/live_request_tests.rs`, which constructs two 500-valued destinations for one owner and expects the request to build, commenting that an even split is an ordinary transfer. The request is accepted, the multiset holds one distinct entry with a count of two, and the total is the sum. There is no fault here for any layer to refuse.

The request is that §15.5 stop listing this row among its faults while keeping `duplicated-source`, which belongs there. As with the zero-valued sponsor row, this workspace has not edited the declaration — which rows §15.5 lists is the guide's to say — and has corrected the row's standing to a first-party fact citing the deciding test, annotating the declaration at its own site so the disagreement is legible.
