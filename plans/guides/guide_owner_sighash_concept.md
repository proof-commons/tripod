...
**Ruling (post-verdict): the issuance dimension is excluded — re-typed from required to refused.** Wave 4's verdict stopped on the one required dimension no candidate this arc builds can exercise: the message's two issuance terms, at lines 2738 and 2739 of the reviewed interpreter, hash one zero byte per input for every issuance-free candidate, so agreement between recomputation and acceptance exercises the input count and nothing about any issuance, and the input count is already carried by the all-inputs dimension. The census refuses an issuance-bearing signing request and the decoder refuses issuance-bearing bytes, so the required typing was the one layer promising evidence the other two layers refuse to admit. The dimension is therefore refused: it joins the single-output and input-extension dimensions as a refusal with content, its enforcing rows are the existing census and decoder refusals, and the follow-up wave lands the re-typing, recomputes the disposition, and clears the review residual on that recomputation. Per the standing exclusion-labeling ruling, the exclusion mints its label in the consensus-exclusions register, the enforcing rows cite the label, and the register section records what a future revision of this exclusion — or any other — would have to touch.
