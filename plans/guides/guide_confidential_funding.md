...
| E5 | evidence | Which Guide-13 rows may enter the Wave-5 delta, and which sponsor rows stay blocked? | RESOLVED — nine of the ten positive private classes may, after the external closure and on observed acceptance; the private-sponsor-values row may not, and the reason it may not has since MOVED. It rested on `SponsorEnvelopeSignerAbsent`, and that residual is CLEARED: a target accepted a sponsor-signed control, which is the condition the blocker's own defining site named. The row's live ground is the explicit/confidential gap — it asks for confidential sponsor VALUES and every sponsored control accepted so far is explicit — and a confidential sponsor value additionally requires a sponsor change output, since a without-change shape forces the sponsor input's value equal to the explicit fee and a commitment to a publicly derivable number hides nothing (§11.2). |
