...
> Wave 2 closed under its recorded bite rows (`T11-013` through `T11-020`); Wave 3 is open under `T11-024` through `T11-032`, with six questions handed up at (`rem:phase6:wave3-questions`).
...
1. Who owns the operator profile and its deployment binding? The alternatives are to extend architecture `DeploymentProfile` now and change its schema and validation, to add a candidate linker binding plus a source-owned tapscript profile while deferring release serialization, or to accept a request/wire profile as evidence, which fails the exit. The recommendation on the merits is the candidate linker binding plus a private revision-bound tapscript establishment type, with transaction accepting that type and a validated census under the existing all-inputs/all-outputs script-path semantics and a STATE-specific protected-data map; this follows the existing owner pattern without turning key rotation into an architecture change (`packages/architecture/src/deployment.rs:68,175–191`; `packages/linker/src/live_deployment.rs:50–88`; `packages/tapscript/src/authorization.rs:471–496,694–805`; `plans/phases/06-state-and-maturity.md:166,176`; `plans/guides/guide_fourteen.md:958–975,1393–1402`).

2. Where may the public test signer's material live, and what does the exclusion forbid? The alternatives are fixed published scalar and auxiliary constants behind a closed handle table, direct reuse of arbitrary-scalar `OwnerSigningMaterial`, which cannot enforce published provenance, or a production signer/key-import service, which needs the separate security design excluded from this wave. The recommendation on the merits is fixed handles resolved inside conformance and Python from committed disposable material, with vectors alone assigning maturity-operator meaning, `FIRST_SCALAR` and `THIRD_SCALAR` serving the positive and wrong-key controls, scalar input absent from wire, argv and configuration, and key files, wallet seeds, production credentials and arbitrary digest signing prohibited (`packages/vectors/src/live_plan.rs:57–83`; `packages/target-elements-conformance/src/test_material.rs:137–171`; `packages/target-elements-conformance/src/constructor/internal_key.rs:5–24`; `scripts/elements-native-executor.py:641–647`; `plans/guides/guide_fourteen.md:532–550,1373–1380,3679–3698`).

3. What does strict response acceptance require? The alternatives are the sponsor's nonempty-stack and present-echo shape, structural operator fields whose claims are trusted, or complete request/result binding with independent verification before authorization. The recommendation on the merits is the third: exactly one result for one outstanding case and its exact input, fixed signer/public key and closed profile; byte-identical finalized echo; one 64-byte Schnorr witness item under the default type; matching leaf, control and deployment census; no success artifact on refusal; and typed rejection of every missing, duplicate, unexpected, alternate, malformed, mismatched or post-finalization mutation case, with native acceptance and protected-term controls remaining additional evidence (`packages/target-elements-conformance/src/protocol.rs:2784–2845,3073–3083`; `packages/transaction/src/live_census.rs:1114–1205,1473–1492`; `packages/transaction/src/live_signing.rs:268–330`; `plans/guides/guide_fourteen.md:1382–1389,2094–2128`).

4. Which interface triggers the protocol revision? The alternatives are wholly in-process signing with existing funding/submission and no wire change, extending the old ECDSA sponsor subject that lacks script-path context, or adding the guide's target-generic script-path capability and exact subject/result. The recommendation on the merits is the third, moving Rust and Python together from revision 7 to 8 while retaining capability negotiation, binding finalized bytes, input index, complete ordered spent outputs, leaf/control data, fixed signer/profile and session genesis, and preserving historical revision-7 parsing and records (`packages/target-elements-conformance/src/protocol.rs:218,437–450,2191–2201`; `packages/transaction/src/live_message.rs:283–315`; `scripts/elements-native-executor.py:4509–4533,510`; `plans/guides/guide_fourteen.md:1351–1371,1391,3675–3677`; `packages/vectors/src/live_corpus_native_v2_r7.rs:1533`).

5. What affine-right guarantee can Wave 3 actually evidence? The alternatives are a non-Clone token with reacquisition and retry undefined, a scoped authority registry and consuming right with explicit outcomes, or a claim that the ledger prevents competing signatures, which confuses signatures with ledger commit. The recommendation on the merits is a registry scoped by deployment, branch/checkpoint, predecessor version and selected operator/profile, holding exact candidate bytes: issuance consumes the right, identical retry returns the cached result without another signing action, replacement is pre-sign only, post-dispatch timeout is indeterminate and cannot reopen the right, and reorganization requires newly evidenced context; its log makes no claim about global signer honesty or signatures outside the registry (`plans/guides/guide_fourteen.md:1986–2019`; `plans/guides/guide_fourteen_conceptual_register.md:23–43,57–61,85`; `packages/transaction/src/live_handoff.rs:200–203`; `packages/target-elements-conformance/src/executor.rs:2050–2118`).

6. How much synthetic construction belongs in this wave, and may selected-profile closure promote generic capability status? The alternatives are to pull the complete STATE constructor, pattern and ABI into Wave 3, to demonstrate operator signing closure on one minimal finalized synthetic script-path candidate while preserving later-wave obligations, or to relabel owner controls and generic Incomplete capabilities as completed maturity evidence. The recommendation on the merits is the minimal candidate with a committed approved operator key and exact protected-byte controls, followed by later STATE constructor/ABI integration; generic Incomplete assessments remain honest, selected-profile evidence stays narrower, and metadata/nonce binding through commitments remains an obligation until the real constructor exists (`packages/target-elements/src/capability.rs:723–737`; `packages/target-elements/src/authorization.rs:797–925`; `packages/transaction/src/live_message.rs:272–318`; `plans/guides/guide_fourteen.md:2941–2944,2952–2980,3040–3068`).
