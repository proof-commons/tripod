...
6. Operator authorization is an operation-level evidence-required relation and an operator constructibility class. The existing relation bodies can state only input-owner authorization or permissionlessness, and the constructibility relation can classify only public-permissionless or owners-of construction (`packages/realization/src/relation.rs:20-25`, `:42-68`); neither fits STATE, whose recognized shape has no owner (`packages/realization/src/evaluate.rs:828-840`), while permissionlessness requires an empty protocol-signer set (`packages/realization/src/evaluate.rs:371-374`) and the model instead requires the operator signer before testing maturity (`packages/model/src/ops/maturity.rs:34-40`). The observation exposes presented signers but no expected operator identity (`packages/realization/src/observation.rs:210-220`), so the honest relation result at this layer is `EvidenceRequired`, following the explicit external-premise shape already used by substrate conservation (`packages/realization/src/evaluate.rs:291-304`), rather than a passed result inferred from an identity the evaluator does not possess. This adds `OperatorAuthorization` to the relation vocabulary and `Operator` to `ConstructibilityClass`; it reuses the graph's existing `ConstructibilityAuthorization::Operator`, its availability discharge, and the exact announce-maturity authorization case (`packages/realization/src/constructibility.rs:34-43`, `:276-289`; `packages/realization/src/validate.rs:493`; `packages/realization/src/tests/constructibility_tests.rs:400-410`). The external requirement records what a later layer must prove; Wave 3 still owns the operator-key encoding and target-native signature closure (`plans/guides/guide_fourteen.md:2925-2935`).

7. Every semantic STATE field used by an announcement relation is a field-level public fact, separately keyed for the input and output side. `FactId` currently has no STATE-metadata key, while `FamilyAmount` is expressly the aggregate amount of an object family (`packages/realization/src/identity.rs:16-77`), so it cannot honestly stand for `omega`, `y_l`, `y_t`, `q`, `cycle`, or `maturity`, whose public six-field census is fixed by `StateMetadata` (`packages/realization/src/state.rs:44-70`). The vocabulary bite therefore adds typed public STATE-field identities rather than an opaque metadata blob or a caller-authored truth value. Their evidence must be either explicit encoding or a public commitment with an authenticated public opening, because D005 admits exactly those two carriers for public values and rejects unauthenticated metadata amount (`plans/decisions/005-value-representation.md:84-97`).

8. STATE admits exactly the public representation pair `Explicit` and `PublicCommitted`. D005's public-value rule permits explicit encoding or a public commitment plus authenticated public opening (`plans/decisions/005-value-representation.md:84-97`), and the compact-ASH declaration already spells that same pair in a `Relation::Representation` body (`packages/realization/src/declarations/compact_ash.rs:337-344`). `PrivateCommitted` cannot carry the field-level public facts required by ruling 7, and no third representation mode is needed to state either admitted carrier.

9. STATE's lifecycle census has exactly six exits: `AdmitDeposits`, `Cycle`, `Redeem`, `ReceiptRelabel`, `Clear`, and `AnnounceMaturity`, the six mutators on its architecture `ObjectSpec` (`packages/architecture/src/spec.rs:2047-2063`). The announcement declaration must state all six lifecycle-exit relations under both admitted STATE representations, and a scope-gated content pin must compare the declared set and the scoped State object's mutator set with that exact census: the generic lifecycle weld deliberately derives its expectation from the declarations and therefore cannot detect a coherent omission from both relation and graph (`packages/realization/src/validate.rs:525-551`, `:635-678`). The announce-maturity architecture mirror separately checks its exact-one STATE input and exact-one STATE output (`packages/architecture/src/spec.rs:1607-1638`) with scoped `StateInput` and `StateOutput` mismatch fields; generic family validation continues to own their cardinality and recognition relations (`packages/realization/src/validate.rs:318-375`), so the operation mirror does not create a second owner for those relations.
...
(b) Guide §3.1 expects the announcement lead bounds in the typed architecture. The tree owns them in the model's `Constants`, with no `BoundId` naming them. Any relation or expression that decides whether an announced cycle lies in the permitted window needs named minimum and maximum inputs, so this is on the critical path for the vocabulary bite and declaration rather than only for later Wave-2 calibration. Whether the architecture gains those bounds — a schema bump under the header rule at `packages/architecture/src/ids.rs:1-5` — or another public typed owner is admitted remains open; the declaration must not substitute unkeyed constants.
...
(e) The relation shape in ruling 6 leaves operator authorization as evidence required because the current observation carries presented signers but not the expected operator identity. The remaining question is whether the target wave should discharge that external requirement from its operator-key encoding and target-native signature control, or whether `OperationObservation` should instead gain an expected operator identity so the realization evaluator can decide the relation directly; doing both would create two owners for the same authorization verdict.
...
The announce-maturity declaration cannot yet be expressed without borrowing the meaning of an existing closed vocabulary. Three gaps are exact and local:

| Gap | Tree evidence | Vocabulary owed before declaration |
|---|---|---|
| operator relation | `Relation` has only `OwnerAuthorization { object }` and `PermissionlessAuthorization` (`packages/realization/src/relation.rs:42-68`). STATE recognition requires no owner (`packages/realization/src/evaluate.rs:828-840`), the permissionless evaluator requires no protocol signer (`packages/realization/src/evaluate.rs:371-374`), and the model requires the operator signer (`packages/model/src/ops/maturity.rs:34-40`). | Add the operation-level `OperatorAuthorization` body in the evidence-required shape fixed by ruling 6, then carry that exhaustive body through the compiler sites at `packages/compiler/src/requirement.rs:541-550`, `source.rs:230-237`, `layout.rs:442-451`, `placement.rs:566-573`, `coverage.rs:763-780`, and `packages/realization/src/tests/compact_ash_tests.rs:1695-1704`. |
| operator constructibility | `ConstructibilityClass` has only `PublicPermissionless` and `OwnersOf` (`packages/realization/src/relation.rs:20-25`), although the graph already has `ConstructibilityAuthorization::Operator`, discharges operator availability, and derives exactly that case for announce-maturity (`packages/realization/src/constructibility.rs:34-43`, `:276-289`; `packages/realization/src/validate.rs:493`; `packages/realization/src/tests/constructibility_tests.rs:400-410`). | Add `ConstructibilityClass::Operator` and map it to the existing authorization case; do not mint a parallel graph authorization. |
| public STATE fields | `FactId` has no key for a STATE metadata field, and `FamilyAmount` is aggregate family value (`packages/realization/src/identity.rs:16-77`); the semantic owner exposes six public fields (`packages/realization/src/state.rs:44-70`). | Add typed input/output identities for each public STATE field, including cycle and maturity, with no conversion of ordinals or maturity variants into family amounts and no caller-authored boolean surrogate. |

The bite split follows those dependency edges. First the vocabulary bite adds the three missing vocabulary families and their exhaustive consumers without declaring the operation. Then the declaration bite adds the announce-maturity module, its exact relation and lifecycle censuses, the scoped architecture mirror and content pins, and corruption tests; that mirror needs only new `StateInput` and `StateOutput` diagnostics in the current mismatch enum (`packages/realization/src/error.rs:12-37`), while the generic validator retains cardinality and recognition (`packages/realization/src/validate.rs:318-375`). Finally the model-conformance bite evaluates the declared announcement against model outcomes and successor projection rather than merely comparing the already-closed semantic transition.

This declaration sits on the Wave-1/Wave-2 boundary because it closes the realization-owned semantic surface that Wave 1 established while furnishing the exact declaration-derived relation census that Wave 2 requires (`plans/guides/guide_fourteen.md:2873-2892`, `:2900-2917`). It does not pull Wave 3 forward: operator-key encoding, independent message recomputation and target-native signature control remain Wave-3 work (`plans/guides/guide_fourteen.md:2925-2935`), and ruling 6 records that separation as an external evidence requirement.
