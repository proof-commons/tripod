...
> are landed except the architecture-owned lead-bound adapter, which lands in the refit bite `T11-021`;
...
Under rulings 11 and 13, `T11-021` is the combined refit bite for architecture-owned lead bounds and side-indexed STATE keys; the altered-lead-bounds corruption check in B5 (`T11-017`) waits on that bite so `packages/compiler/src/maturity_announcement_requirements.rs` can consume both keyed bounds and keyed successor laws.
