...
| `T5-023` | DOING | Intermediate confidential-funding guide, Wave 2, proving one confidential predecessor: the fixture registry, handle grammar, drift digest, and deterministic derivation recipe land as the only lookup authority; one deterministic funding transaction is materialized to the exact hybrid shape the guide fixes — one explicit protocol-asset input contributing a zero value blinder, exactly two protocol outputs carrying the explicit asset and positive confidential values summing to it, zero asset blinders and therefore no surjection proof, two ordered additive-inverse value blinders, a bounded deterministic counter search yielding the two admitted commitment parities in fixed order, a deterministic nonce per output, and a valid rangeproof per output bound to that output's commitment, the unblinded asset generator, and the program — with any policy-asset fee or change member classified into an explicit non-protocol region outside both balance equations; the transaction is submitted, accepted, mined, and read back raw, and recomputation and readback agree on all eight members of the funding agreement census for both outputs, always between two distinct origins and never between a value and itself; the validated funding record and its recomputing validator carry every non-claim; and the confidential-predecessor blocker is removed from the carried residuals and from the rows that carried it, at that boundary only and after validated readback only, moving exactly one residual and clearing neither owner-sighash blocker |
