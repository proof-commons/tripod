...
| `T5-043` | TODO | Sponsor wave, runtime-threaded reserve asset and the observed sponsor-signed submission: the residual SponsorEnvelopeSignerAbsent stands because no sponsor-signed control has an OBSERVED result, and exactly one thing blocks a submission — the live deployment pins its reserve asset to a fixture constant while the chain's own reserve is knowable only from the funding answer. The reserve becomes a runtime parameter beside the protocol asset, the live lane gains the sponsor-input reserve guard the compact-ASH lane already enforces, and the sponsor-signing lane settles the reserve from the sponsor-funding response before it links, then submits the sponsor-signed control and records whatever the target answers. IN PROGRESS |
