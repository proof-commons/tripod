...
| `T11-016` | BLOCKED | B4 is blocked on `T11-021`: add the plan boundary in new `packages/compiler/src/maturity_announcement_plan.rs`, a bespoke validated container reusing the shared generic row vocabulary of `packages/compiler/src/operation_plan.rs`, with fields and helpers for both representation modes, every factor agreeing, a direct declaration census, and no public unchecked construction; add the exports, error wiring, and census entries. Estimate: 1,200–1,600 lines. |
| `T11-017` | BLOCKED | B5 is blocked on `T11-016` (B4) and `T11-021`: add validators and corruptions across `packages/compiler/src/maturity_announcement_plan.rs`, `packages/compiler/src/maturity_announcement_requirements.rs`, and new `packages/compiler/src/tests/maturity_announcement_plan_tests.rs`. Require full source/projection/case/coverage comparison, cross-level evidence closure, the corruption list — omission, duplication, extra identity, body, case mode, activity, coherent relation/dependency omission, and altered lead bounds after `T11-021` resolves their fact keys to architecture bounds — plus the supplemental obligation/dependency census and sponsor traversal. Estimate: 250–400 production and 500–750 test lines. |
| `T11-018` | BLOCKED | B6 is blocked on `T11-016` (B4): add an independent announcement oracle test beside `packages/compiler/src/tests/coverage_oracle_tests.rs` and `packages/compiler/src/tests/analyzed_program_oracle_tests.rs` and the pilot-program matrix in `packages/compiler/src/tests/pilot_program_tests.rs`. Its expectations state the literal 26-relation/23-edge census, the independent four-case product, exact expected rows, and permutation and corruption checks, with no production derivation in expectations. Estimate: 400–650 lines. |
| `T11-019` | BLOCKED | B7 is blocked on `T11-017` (B5): add consumer-only success, error, equality, census, opacity, and leakage coverage to `packages/compiler/tests/public_api.rs`, with no internal analysis access. Estimate: 180–280 lines. |
| `T11-020` | BLOCKED | B8 is blocked on `T11-019` (B7): update the tour and stale counts in `packages/compiler/README.md`, correcting the external-role count and public-entry count against the code and distinguishing requirements from discharged evidence. Estimate: 40–70 lines. |
| `T11-021` | TODO | Combined lead-bound and side-indexed STATE-fact refit. Architecture: append two `BoundId` entries for the announcement lead minimum and maximum after `FeeSponsorInputMax` in `packages/architecture/src/ids.rs`; declare both deployment-calibrated with no draft default in `packages/architecture/src/spec.rs` beside the announce-maturity row's existing bound; move the schema constant from 17 → 18; re-measure and re-pin the semantic and behavioural hashes with the denotation record in `packages/architecture/src/tests/versioning_gate_tests.rs`, the register records in `plans/registers/identities.md`, the generated publications under `packages/model/generated/`, and the attestation masthead regenerated; and add a lead-pair validity check — the minimum is positive and at most the maximum — in deployment validation in `packages/architecture/src/deployment.rs`. Model: make bound resolution unit-safe because the existing resolver in `packages/model/src/manifest.rs` returns a count for every bound while announcement leads are cycles, with count consumers rejecting cycle bounds and conformance covering both units; prove the `min_maturity_lead` and `max_maturity_lead` constants in `packages/model/src/constants.rs` equal the architecture-keyed values by conformance test. Realization: give `FactId::StateField` the transaction side; make the announcement declare fifteen public facts; give the `AnnouncementLeadBound` selector an exhaustive mapping to the two bound identifiers rather than adding bound-value facts; feed the validated lead bounds from architecture-keyed values through a typed adapter; and make the missing-carrier arm in `packages/realization/src/evaluate.rs` cover both sides. Compiler: refit `packages/compiler/src/maturity_announcement_requirements.rs` to six field requirements of input key, output key, and law, with a payload-free law-kind census covering all six exits, an operand list naming further facts or the published cycle-split parameter, fifteen public facts, STATE succession as input and output key sets, public recovery as seven source keys and six result keys with six ancillary roles, and the symbolic successor roles removed. Census re-pins: move the compiler disclosure census from 9 → 15 while reviewing and retaining every other per-layer figure; update the requirement-module tests in `packages/compiler/src/tests/maturity_announcement_requirements_tests.rs`; move the model conformance census in `packages/model/src/tests/realization_conformance_tests.rs` from 9 → 15 public facts; move the realization declaration tests in `packages/realization/src/tests/announce_maturity_tests.rs` from 9 → 15 disclosure nodes; and add a consumer test for the exported requirement vocabulary in `packages/compiler/tests/public_api.rs`. Update `packages/realization/README.md` and `packages/compiler/README.md`. Estimate: 400–800 production and 400–700 test lines plus the regenerated publications. Must land before `T11-016` (B4) and `T11-017` (B5). |
