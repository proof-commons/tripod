...
| `T11-012` | TODO | Wave-1 batch boundary opened in `0.6.149-dev`: run the full `scripts/ci.sh` gate on the compute server at the Wave-1 closing tip after this record lands, then record its patch tag, exit status and measured wall time here. The scoped documentation gate used to validate these closeout edits does not substitute for the batch boundary; this row remains TODO until the full gate has run. |
