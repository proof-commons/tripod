...
| `T5-024` | DOING | Intermediate confidential-funding guide, Wave 3, materializing private transactions transaction-wide: the transaction-wide materializer, its intent, view, and profile types, the preflight and the seven ordered stages, the four serialization changes and the protected-bytes repair, the proof-finalized candidate and its region census, the third origin-tagged commitment newtype and the independence rule, and the retirement of the per-output role from private complete-transaction claims |
