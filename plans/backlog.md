...
| `T11-031` | DONE | Synthetic operator/native evidence sweep. Add `packages/vectors/src/maturity_operator.rs` and `packages/vectors/tests/maturity_operator.rs`, registered by `packages/vectors/src/lib.rs` and `packages/vectors/meson.build`, with the independent verifier adapter in the new vectors module using `packages/target-elements-conformance/src/owner_key_oracle.rs:238–282`; freeze one synthetic spend with a committed approved operator key, use the generic signer, insert its validated witness, submit/read back through native operations, and derive private evidence from exact transcripts, recomputed messages, key/profile/input/deployment binding and the model signer projection while limiting metadata/nonce claims to commitments actually present. Gate a native positive, declared-boundary negatives, byte-identical readback, independent signature verification, the model membership decision and evidence-incomplete standing for every unanswered row, including wrong key/candidate/leaf, unknown-key admission, signature width/type, protected-term movement and duplicate authorization. Estimate 450–700 production / 300–500 test lines. Depends on `T11-029`; rulings 16–21 at (`rule:phase6:wave3-rulings`) settle the six handed-up questions. The node-free half landed in `0.6.166-dev` as one commit changing seven files by +3,996/−2,028: the shared capture writer moved into `packages/vectors/tests/common/`, the synthetic spend binds the third published signer distinctly from both demonstration owners, the native plan fixes eleven steps, and its derived evidence record leaves operator membership external with twelve STATE facts unconstructed. Twenty-five tests pass beside one ignored native carrier, moving vectors from 529 to 554 passed. The first native run funded the predecessor, obtained both signatures and saw all six controls refused at the declared script-path layer, but the positive received the same "Invalid Schnorr signature" verdict although the independent verifier accepted it; the kernel and adapter agreed with each other and not with the target, and target-framework recomputation located the divergence in deployment-genesis byte order. The repair landed in `0.6.167-dev` as one commit changing six files by +114/−13: the printed identity is reversed at the operator fixture's kernel handoff, the adapter reads the printed octets big-endian for the target's internal serializer, and the handoff binding compares the reversed printed identity with the internal deployment bytes; non-symmetric-genesis regressions distinguish the orders. Transaction unit tests moved from 339 to 340 and vectors from 554 to 555 passed; the native carrier then accepted and read back the operator positive byte-identically while all six controls remained refused. The operator capture driver and contract landed in `0.6.168-dev` as 672- and 470-line siblings of the unchanged revision-8 live-transfer driver pair: they enforce a one-ceremony census, revision-8 checks, invoking-variable run identity and 45 contract cases, while the shared writer gains the revision-8 signing capability in one line. The run of record landed in `0.6.169-dev`: four byte-identical evidence files under `packages/vectors/fixtures/native-operator-run-of-record/` — capture 76,311 bytes, timing 49, manifest 187 and report 1,933 — sit behind a 1,457-line strict revision-7-pattern importer that reconstructs funding subjects, replays the planner and refuses any recorded standing that differs from derivation, with a 33-line replay entry point, an 8-line conformance response-decoding export and one hash-citation-family rule for the suite-provenance fields. Thirty-three added Rust tests move vectors from 555 to 588 passed. The derived record answers ten of eleven rows: the positive is a native observation whose identity equals both readback and recomputed transaction identity; wrong key, wrong candidate, signature type and protected term carry "Invalid Schnorr signature", wrong leaf carries "Witness program hash mismatch", and signature width carries "Invalid Schnorr signature size", all at the script-path boundary; unknown-key admission, duplicate authorization and signature verification are first-party facts. Operator membership alone remains an external evidence requirement with the twelve STATE facts unconstructed because the realization crate exposes no decision over an observed authorization. |
| `T11-032` | DONE | The Guide-14 Wave-3 closure and handoff lands through `0.6.170-dev` and `0.6.171-dev`: the workspace rustdoc lane closes finding 14; the three package READMEs record the revision-8 native protocol, exact target and operator profile standing, and native operator evidence; the source comments distinguish the established selected profiles from generic `Incomplete` capability status and point owner census assembly at the shared script-path kernel; the Phase-6 card records all nine bites, findings 14–21 and the constructor/ABI inheritance; and the conceptual register adopts R-4 and closes `G14C-05` from the affine registry and admitted native evidence (`0.6.163-dev`, `0.6.169-dev`). The strict native revision-7 importer and strict native revision-8 operator importer remain exercised together by the vectors lane in every gate of record, so the established pilot corpus stays gated beside the new corpus; the admission added 33 Rust tests and moved vectors from 555 to 588 passed tests with the ignored count unchanged (`0.6.169-dev`). The nightly gate-of-record chain was all green: the carrier ran 51 lanes in 643 s (`0.6.166-dev`); the capture driver, gated with the genesis repair, ran 51 lanes in 571 s (`0.6.167-dev`, `0.6.168-dev`); admission ran 51 lanes in 1352 s under a load average near 45 (`0.6.169-dev`); the untagged documentation close-out preceding the pending base ran 51 lanes in 481 s; and the rustdoc landing was the first 52-lane gate, green in 574 s, with its workspace-rustdoc measurements recorded as 70.57 s for a first shared-cache run and 0.26 s for a warm repeat (`0.6.170-dev`). This row's own landing-gate figure follows in the row's import record. |
| `T11-033` | DONE | The codec field-slicing commutation bite landed through `0.6.172-dev`: `packages/realization/src/tests/state_codec_tests.rs` gained a typed table of the seven replaceable field slices in the 86-byte STATE metadata encoding — the five eight-byte scalars at offsets 25, 33, 41, 49 and 57, the nine-byte maturity field at 65, and the four-byte nonce at 74 — with the reserved eight-byte tail at 78, plus four tests covering exact tiling, canonical and strict maturity bytes, independently computed field bytes with exact differences, and single-field replacement by slice splicing. One test file changed by 194 added lines, adding four tests; the realization crate's unit tests moved from 323 to 327. Under a load average near 50, the lane gate ran 2 min 57 s; the gate of record ran 52 lanes green in 849 s. This discharges the codec-law part of deliverables 1 and 11 and `G14C-14`, and rests on ruling 30 at (`rule:phase6:wave4-rulings`). |
| `T11-034` | DONE | The Wave-4 constructor core landed through `0.6.174-dev`: new `packages/tapscript/src/state_constructor.rs` (1,075 lines) and `packages/tapscript/src/tests/state_constructor_tests.rs` (903 lines, 36 tests), the manifest's production `realization`, `sha2` and `thiserror` dependencies, exports, test registration and the Meson census changed six files by +2,001/−26. The closed sum has eighteen refusals; only `TweakAboveGroupOrder`, `TweakedPointIsIdentity` and `CanonicalBranchSideNotSatisfied` are retryable. The `0..4096` nonce search records every rejected lower candidate, and the golden fixture selects nonce 1 after the branch side refuses nonce 0, the NUMS key is recomputed on admission, control recipes enforce the 128-node limit, each transaction side has six field commitments, and seven typed reference declarations feed a component census that refuses cycles. Tapscript unit tests moved from 444 to 480; under a load average near 50, the lane's final gate round ran 196 s, and the gate of record ran 52 lanes green in 1168 s. The landing rests on rulings 22–26 and 28–30 at (`rule:phase6:wave4-rulings`) and discharges the constructor-core portions of deliverables 1–10 and the abstract boundary of all five exits. It made the oracle cross-checks in `T11-035`, the linker graph completion in `T11-036` and the anchor in `T11-037` dispatchable. |
| `T11-035` | DONE | The Wave-4 oracle cross-checks landed through `0.6.178-dev`: a real-arithmetic curve capability over the prototype's field arithmetic keeps the byte, root, commitment, generated-case and NUMS claims as separate tests, and the production leaf runs under the abstract oracle for empty, true, false, arbitrary and depth-edge initial stacks. The reference gained an independent Boolean-verification contract, whose first form the agreement test corrected by adding the target-domain abort that the production executor reports. The cross-check proves production nonce = prototype attempts − 1. Three files changed by +517/−26: the two test files plus the test-only `realization` manifest line; tapscript rose from 480 to 481 tests and target-elements-conformance from 611 to 615. The lane's final gate was green in 396 s under a load average near 50, and the gate of record ran 52 lanes green in 820 s. The landing rests on rulings 23, 26, 29, 30 and 32 at (`rule:phase6:wave4-rulings`) and discharges deliverable 11 and exits 1, 2, 4 and 5 on abstract and oracle evidence without promoting the prototype schema. |
| `T11-036` | DONE | The constructor reference graph landed through `0.6.176-dev`: new `packages/linker/src/state_constructor_graph.rs` (258 lines) freezes canonical nodes and dependent-to-dependency edges from the constructor's typed declarations, normalizes strongly connected components to their smallest reference with their internal edges, bounds distinct references at the constructor's own 64, and refuses every cyclic component at once without selecting a cut. New `packages/linker/src/tests/state_constructor_graph_tests.rs` (327 lines, 12 tests) covers permutation invariance, canonical component identity, duplicate and split declarations, edge direction with undeclared nodes, a self-loop, a three-cycle inside an acyclic set, several cycles in one refusal, the empty graph, the bound, and a real constructor whose seven declarations give seven singleton components equal to the constructor's census. Five linker files changed by +593; linker unit tests moved from 112 to 124. Under a load average near 50, the lane's final gate ran 181 s; the gate of record ran 52 lanes green in 758 s. The landing rests on ruling 28 at (`rule:phase6:wave4-rulings`), discharges the Wave-4 portion of deliverable 10, and reserves `G14C-08` for Wave 6. |
| `T11-037` | DONE | The state-thread anchor landed through `0.6.177-dev`: `StateThreadAnchor` in `packages/transaction/src/operator_right.rs` has private fields and fallible construction carrying deployment identity, the checked branch context, the starting outpoint, closed asset-or-issuance provenance, the constructor generation named by tapscript's `StateConstructorGeneration`, a closed checkpoint policy of the exact branch or at or beyond an ordinal, a continuity-evidence status that remains outstanding in this wave, and a synthetic-or-observed origin. `check_continuations` refuses out-of-policy candidates and a second accepted continuation per branch identifier as equivocation, returning at most one accepted continuation per selected branch with the origin unpromoted. Tapscript became a direct production dependency of transaction, with the admission recorded in `plans/packages/transaction.md`. Sixteen tests landed with 355 test lines and 240 production lines; transaction unit tests moved from 340 to 356. Under a load average near 50, the lane's final gate ran 292 s; the gate of record ran 52 lanes green in 681 s. The landing rests on ruling 27 at (`rule:phase6:wave4-rulings`) and supplies the evidence pointer for `R-3 ANCHOR` and `G14C-04`, whose adoption is `T11-040`'s decision. |
| `T11-038` | DONE | The membership seam landed through `0.6.173-dev`: the witness, disposition and provenance-error types, the `OperatorMembership` failure class, the optional witness on the internal evaluator, the new scoped entry point, and a test-only wrapper that kept existing tests untouched landed with seven tests. Four realization files changed by +322/−22; the realization crate's unit tests moved from 323 to 330, with 13 doctests unchanged. Under a load average near 50, the lane gate ran 5 min 5 s; the gate of record ran 52 lanes green in 1198 s. This discharges the inherited evaluation obligation of the Wave-3 closure and none of the eleven deliverables, and rests on ruling 31 at (`rule:phase6:wave4-rulings`). `packages/realization/README.md` records the vocabulary from this landing. |
| `T11-039` | DONE | Writer retention landed through `0.6.175-dev`: the shared capture writer in `packages/vectors/tests/common/mod.rs` writes `request-subject <len> <hex>` immediately after `request-bytes`, using the protocol serializer's bytes of the typed subject alone, and advances the first line to `native-capture-schema 2` so a schema-1 reader refuses a new capture immediately. The exact-golden-bytes test moved with the grammar, and a test-only `serde_json` dependency plus three node-free tests cover round trips over the four funding subject kinds and a submission, one subject line per operation block in protocol order, and submission redundancy. Four vectors files changed by +269/−8; vectors moved from 588 to 591 passing tests. Under a load average near 50, the lane's gate ran 6 min 28 s; the gate of record ran 52 lanes green in 733 s. The landing rests on ruling 33 at (`rule:phase6:wave4-rulings`) and discharges the lossless-plumbing half of Wave-3 finding 20; the ceremony half remains a Wave-8 dependency, together with teaching the live driver script the new schema, which still pins schema 1. Recorded fixtures remain byte-identical at schema 1. |
| `T11-040` | DONE | The Guide-14 Wave-4 closure and handoff lands through `0.6.179-dev`: the Phase-6 card records the closure paragraphs, findings, deliverable and exit verdicts, and later-wave handoffs; the conceptual register adopts `R-1 NONCE` and `R-3 ANCHOR` and records the Wave-4 portions and resulting row statuses; the tapscript package card and tapscript, linker and transaction READMEs describe the landed constructor, frozen graph and typed anchor; and the transaction and vectors manifests restore the dependency-boundary and capture-serializer comments. The two target-native metadata-leaf rows and Wave 8's constructor-carrier ceremony, Rust test identity and live driver's schema update remain recorded as outstanding. |
| `T11-041` | IN PROGRESS | Guide-14 Wave-5 opening record and stale source-comment correction. `plans/phases/06-state-and-maturity.md` records the fourteen-deliverable and four-exit gap matrix, the 26-relation, 23-edge and 15-fact ground truth, rulings 34–39 at (`rule:phase6:wave5-rulings`), findings 26–30 at (`sec:phase6:wave5-findings`) and this eight-bite plan; `plans/backlog.md` opens `T11-041` through `T11-048`; and the two stale comment lines in `packages/vectors/src/maturity_operator.rs` name realization's observed-operator-membership seam without claiming this receipt candidate supplies a witness. Shape: documentation and comment only, about 120 documentation lines plus the two comment lines, with no implementation dependency beyond the Wave-5 study and the closed Wave-4 handoff. The record rests on rulings 22–33 at (`rule:phase6:wave4-rulings`), establishes rulings 34–39, discharges every opening design choice but no implementation deliverable, and gates on check-plans, Meson lint, label tests, workspace Clippy and vectors rustdoc before the gate of record. |
| `T11-042` | TODO | STATE pattern record and structural fragments. Add `packages/tapscript/src/state_pattern.rs` with the STATE-specific record and one component recipe required by ruling 36 plus five structural fragments: coordinator role, exact cardinality, predecessor recognition by asset, amount, program and current-root role, sponsor isolation, and issuance and absence closure; add `packages/tapscript/src/tests/state_pattern_tests.rs` and register both through `packages/tapscript/src/lib.rs`, `packages/tapscript/src/tests/mod.rs` and `packages/tapscript/meson.build`. Estimate: about 700 production and 800 test lines. Dependency: no dependency beyond the landed `T11-041` record. It rests on rulings 34, 36 and 37 at (`rule:phase6:wave5-rulings`), discharges deliverables 1, 2, 3, 10 and 11 and the record half of deliverable 13, and gates on tapscript tests followed by the gate of record. |
| `T11-043` | TODO | Semantic fragments, blocked on `T11-042`. Add the sibling `packages/tapscript/src/state_announcement.rs` and its own `packages/tapscript/src/tests/state_announcement_tests.rs`, keeping `state_pattern.rs` responsible for the record type and structural fragments: authenticate metadata against the constructor's canonical bytes, check the maturity predecessor, enforce the lead window with checked additions and inclusive comparisons, derive copy-through from the schema, and reconstruct successor output 0. Estimate: about 600 production and 700 test lines. It rests on rulings 23 and 30 at (`rule:phase6:wave4-rulings`) and rulings 36 and 37 at (`rule:phase6:wave5-rulings`), discharges deliverables 4–8, and gates on tapscript tests followed by the gate of record. |
| `T11-044` | TODO | Operator fragment and membership producer, blocked on `T11-042`. Add the tapscript operator fragment with a committed key that is never witness-selected, exactly one signature item, its verifying form and no surviving Boolean; add `packages/transaction/src/operator_membership.rs` with ruling 35's producer; update `packages/transaction/Cargo.toml` and `plans/packages/transaction.md` to admit `realization`; register and test the transaction module; and add seam cases in `packages/realization/src/tests/announce_maturity_tests.rs`. Estimate: about 300 production and 500 test lines. It rests on ruling 31 at (`rule:phase6:wave4-rulings`) and ruling 35 at (`rule:phase6:wave5-rulings`), discharges deliverable 9 and the operator relation of exit 2, and gates on tapscript, transaction and realization tests followed by the gate of record. |
| `T11-045` | TODO | Composed program, production subtree and constructor re-run, blocked on `T11-043` and `T11-044`. Compose the announcement program with metadata unions derived from its component recipe, walk its declared witness, audit its final stack and resources, construct ruling 34's production `StateStaticSubtree` from the composed leaf, and re-run predecessor and successor constructor derivation and golden tests with every constant recomputed. Estimate: about 400 production and 600 test lines. It rests on ruling 24 at (`rule:phase6:wave4-rulings`) and rulings 34, 36 and 39 at (`rule:phase6:wave5-rulings`), discharges deliverables 12, 13 and 14 and exits 1, 3 and 4, and gates on tapscript tests followed by the gate of record. |
| `T11-046` | TODO | Assessment and carrier projection, blocked on `T11-045`. Update `packages/tapscript/src/maturity_assessment.rs` and its tests to replace the no-approved-pattern result only for groups backed by complete records and to project each of the 26 relations to an emitted carrier or one of the two named external roles. Estimate: about 260 production and 500 test lines. It rests on ruling 38 at (`rule:phase6:wave5-rulings`), completes exit 2, and gates on tapscript tests followed by the gate of record. |
| `T11-047` | TODO | Adversarial closure tests, blocked on `T11-046`. Add about 900 test-only lines across the three Wave-5 tapscript test files, exhausting wrong witness shapes, Boolean and signature survivors, dependency deletion, carrier duplication and omission, every pattern refusal, lead boundaries and overflow, copy-through mutation, sponsor amount reads, issuance on every input, final-stack residue, every bounded resource and the recomputed goldens. It changes no production code, rests on rulings 34–39 at (`rule:phase6:wave5-rulings`), reconfirms all fourteen deliverables and four exits, and gates on tapscript tests followed by the gate of record. |
| `T11-048` | BLOCKED | Wave-5 closure and handoff, blocked on `T11-042` through `T11-047`. Add about 60–100 documentation lines to the Phase-6 card and backlog recording closure evidence, row patch tags and final figures, the Wave-6 unresolved-symbol handoff, the Wave-7 membership-byte handoff and the deferred Wave-8 and Wave-12 evidence. It rests on rulings 34–39 at (`rule:phase6:wave5-rulings`), closes `T11-041`, claims only deliverables and exits established by the completed bites, and gates on documentation validation followed by the gate of record. |
