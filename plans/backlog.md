...
| `T11-033` | TODO | Codec field-slicing commutation. Extend `packages/realization/src/tests/state_codec_tests.rs` with field-slicing/semantic-projection commutation over all six fields and all maturity variants using the published offsets and the existing property strategy. This is 0 production / 60–90 test lines, has no dependency, discharges the codec-law part of deliverables 1 and 11 and `G14C-14`, and rests on ruling 30 at (`rule:phase6:wave4-rulings`). Gate: realization package tests, then the full gate of record. |
| `T11-034` | BLOCKED | Constructor core; blocked on `T11-033`. Amend `packages/tapscript/Cargo.toml` to admit production `realization` and the workspace's `sha2` with the rationale in ruling 22 and the companion admission in `plans/packages/tapscript.md`; add `packages/tapscript/src/state_constructor.rs` with private, non-`Default`, fallibly built `CandidateStateConstructor`, `StateConstructorGeneration`, `StateStaticSubtree`, `StateLeafRole`, `StateControlRecipe`, the canonical-side, search, internal-key and totality policies, the exact eighteen-name refusal sum of (`def:guide14-exec:constructor-refusals`) with its retryable/nonretryable split, the metadata leaf builder in the three-instruction form, tagged hashes, typed reference declarations, predecessor and successor recipes, the twelve field-slice commitments and a curve-capability trait, importing nothing from conformance; register it in `packages/tapscript/src/lib.rs`, `packages/tapscript/src/tests/mod.rs` and `packages/tapscript/meson.build`; and add `packages/tapscript/src/tests/state_constructor_tests.rs` with golden leaf/program/root/control bytes, deterministic repetition, first/late/exhausted nonce cases, zero/above-order/identity tweak cases through a scripted capability, every refusal, role/path/depth checks, recipe equality, schema non-leak, the twelve slices, declaration determinism and the key-path non-claim. This is 530–730 production and configuration / 520–700 test lines, rests on rulings 22–26 and 28–30 at (`rule:phase6:wave4-rulings`), and discharges deliverables 1–10 and the abstract boundary of all five exits. Gate: tapscript package tests, then the full gate of record. Future implementation commit subject: `tapscript: implement the candidate STATE constructor`. |
| `T11-035` | BLOCKED | Oracle cross-checks; blocked on `T11-034`. Extend `packages/tapscript/src/tests/abstract_oracle_tests.rs` with the production metadata leaf under empty, true, false, arbitrary and limit-edge initial stacks; add `realization` as a test-only dependency of `packages/target-elements-conformance/Cargo.toml`; and extend `packages/target-elements-conformance/src/tests/constructor_tests.rs` to compare production STATE leaf, branch, tweak, output, parity and control results with the prototype's independent arithmetic for fixed and property-generated canonical bytes, recompute the NUMS derivation, and keep byte, root and hidden-content claims distinct. This is 2–6 production/configuration / 260–380 test lines, rests on rulings 23, 26, 29, 30 and 32 at (`rule:phase6:wave4-rulings`), and discharges deliverable 11 and exits 1, 2, 4 and 5 without promoting the prototype schema. Gate: tapscript and target-elements-conformance package tests, then the full gate of record. |
| `T11-036` | BLOCKED | Constructor reference graph; blocked on `T11-034`. Add `packages/linker/src/state_constructor_graph.rs` consuming the typed declarations, freezing canonical nodes and edges, computing deterministic strongly connected components and refusing unresolved cycles without selecting or accepting a cut; add `packages/linker/src/tests/state_constructor_graph_tests.rs` covering permutation invariance, canonical component identity, acyclic and cyclic declarations, and refusal before cut acceptance; and register both through `packages/linker/src/lib.rs`, `packages/linker/src/tests/mod.rs` and `packages/linker/meson.build`. This is 248–348 production and configuration / 220–300 test lines, rests on ruling 28 at (`rule:phase6:wave4-rulings`), discharges the Wave-4 portion of deliverable 10, and reserves `G14C-08` for Wave 6. Gate: linker package tests, then the full gate of record. |
| `T11-037` | BLOCKED | State-thread anchor; blocked on `T11-034`. Extend `packages/transaction/src/operator_right.rs` with private-field `StateThreadAnchor`, explicit synthetic/observed provenance standing, constructor generation, checkpoint policy, continuity-evidence status and the conditional at-most-one-continuation predicate; extend `packages/transaction/src/tests/operator_right_tests.rs` with construction refusals, branch/network/outpoint/asset/generation distinctions, the conditional theorem and explicit no-genesis tests. This is 180–260 production / 180–260 test lines, rests on ruling 27 at (`rule:phase6:wave4-rulings`), and supplies the evidence pointer for `R-3 ANCHOR` and `G14C-04`. Gate: transaction package tests, then the full gate of record. |
| `T11-038` | TODO | Membership seam. Extend `packages/realization/src/evaluate.rs` with a typed observed-authorization membership input and a fail-closed decision that keeps the relation external until a witness is supplied; extend `packages/realization/src/tests/operator_vocabulary_tests.rs` with member, non-member, missing and mismatched cases plus a tripwire that an absent witness remains external evidence. This is 70–120 production / 120–180 test lines, has no constructor dependency, rests on ruling 31 at (`rule:phase6:wave4-rulings`), and discharges the inherited evaluation obligation of the Wave-3 closure and none of the eleven deliverables. Gate: realization package tests, then the full gate of record. |
| `T11-039` | TODO | Writer retention. Extend `packages/vectors/tests/common/mod.rs` to retain the exact funding request-subject bytes, with node-free round-trip and census tests. This is 0 production / 90–140 test-support lines, has no dependency, rests on ruling 33 at (`rule:phase6:wave4-rulings`), and discharges the lossless-plumbing half of Wave-3 finding 20, the ceremony half being a Wave-8 dependency. Gate: vectors package tests, then the full gate of record. |
