...
| `T5-033` | DOING | Confidential-funding execution guide, Wave 4, closing the external sighash handoff: the proof-finalized candidate's bytes enter the signing request unchanged and are checked so by exact-byte identity rather than trusted, the four handoff states become type transitions with each of the four typed refusals reached by a test, every required owner signs the same protected candidate, and one sponsorless private candidate reaches submit-ready through the mandatory order end-to-end. No digest is designed here and none crosses the boundary in either direction; the output-witness commitment is the parallel sighash work's to test and is cited rather than duplicated. Submission and the evidence restart belong to the next wave |
