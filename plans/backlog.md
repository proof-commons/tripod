...
| `T11-023` | TODO | Two one-line refinements ride with the next code bite rather than standing alone: drop the redundant re-analysis from `plan_maturity_announcement_target_operation` in `packages/compiler/src/maturity_announcement_plan.rs`, whose entry runs its analysis-bound validation and then calls the public validator that analyzes again; correct the module comment in `packages/realization/src/declarations/announce_maturity.rs`, which still counts nine public facts although the declaration publishes fifteen. |
