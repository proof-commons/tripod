...
| `T4-013` | TODO | The `maximum` positive fixture asks for more value than the target admits. Its ASH inputs are `(1 << 51) - 2` and `1`, and the first alone is against an Elements money bound of, so no ceremony can fund it and no submission of it can be accepted. The amount came from the protocol's own 51-bit amount domain rather than from the target's bound; the two are different ceilings and the fixture census currently reads only the first. Decide whether the class is retargeted to the largest amount the target admits, or kept and reclassified as a target-refusal case |
...
Phase-4 work:
    ...
    real. It waits on T4-013: the fifth sponsorless census row asks for
    an input of, the target refuses an output that
    large as bad-txns-vout-toolarge, and one unfundable row stops the
    whole plan because a funding answer that is not an acceptance
    refuses it outright. Reaching a submission needs that row retargeted
    or the plan taught to carry a row the target's bound forbids
```
