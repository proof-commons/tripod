...
| `T5-031` | DONE | Owner-sighash parallel work, Wave 5, handing the accepted result across to the proof-bearing lane: one owner authorization was produced over a candidate whose two output-witness entries carry four thousand one hundred and seventy-four range-proof bytes each, submitted to a real Elements node, ACCEPTED at transaction identity in block six, and re-verified against an independently recomputed message that the read-back signature satisfies and the emptied-vector message does not. The census is built through the proof-finalized route and through no other, over the transaction-wide materializer's own frozen candidate, with the output-witness vector at its real proof-bearing length rather than at a length a caller chose — the deliverable's own figure is written into the artifact as the vector's length and as each entry's proof-byte count, so a run that had somehow authorized an emptied vector would say so in its own bytes rather than in a reader's inference. The spent-output triple signed over is the node's own report of the mined predecessor and never the ceremony's expectation, which on this lane is the difference between a thirty-three byte commitment and an eight-byte amount: the ceremony's expectation is recorded beside the node's and compared, and the two ways of getting that wrong are the wave's two construction controls rather than paragraphs. What the acceptance rests on is scoped rather than absorbed. The funding, the materialization and the blinding belong to the confidential-funding guide, are called rather than checked here, and are disclaimed in the record as data and in the artifact as lines, so a consumer that dropped the sentence would be dropping a value. The leaf spent is a bare owner signature check and not a receipt constructor, because the private branch of the live finalization does not take the transaction-wide path and supplying it an intent, a fixture view and two collaborators is the evidence lane's own restart; that narrowing is stated in the module, in the record's non-claims and in the artifact, and what the run evidences is the MESSAGE the target forms for a proof-bearing candidate and the profile it is taken under, not a covenant, not constructor recognition and not conservation arithmetic. The negative controls are the private lane's own and are designed from the two things that differ here rather than renamed from the explicit lane's. Emptying the output-witness vector moves term twelve, whose entries on the explicit lane are two zero bytes recoverable from the output count in the protected bytes and here are proof bytes recoverable from nothing at any length, so the same-shaped control is a different claim and the module says which. The preimage-only signer moves terms ten and twelve together, which is one mistake and not two, and is kept separate so a refusal cannot be attributed to the output side when the input side moved with it. The another-candidate control moves the successor fixture, and the control states what this lane makes unavoidable: a confidential amount does not move without its commitment and a commitment does not move without the proof built over it, so terms eleven and twelve move together and no smaller change to a confidential amount exists. All three were refused by the target at the script-path layer with the node's own words, mandatory-script-verify-flag-failed and an invalid Schnorr signature, over identical submitted bytes — one candidate, four messages, so a refusal is attributable to the term that moved rather than to a different transaction. The two construction controls are refused before any message is formed and are recorded at the construction layer and never merged with a verdict: declaring the spent value as the amount the ceremony asked for, and binding the input to the other predecessor output's opening, both draw the materializer's opening-mismatch refusal, and the two are distinguished by which side of that equality moved rather than by two variant names the vocabulary does not have. The protected-bytes repair is exercised from the signing side, where the consequence lands on an owner: a census assembled over a candidate whose range proof moved by one byte refuses the bytes the owner is still holding, the witnessless serialization of the two candidates is byte-identical so a census bound to that reading would have let the owner bind to a preimage the target's digest does not cover, and the refusal arrives at the binding check while the two candidates' messages are shown to differ, so the refusal is demonstrated to have had something to prevent. The observation record improves on the Wave-3 gap the Wave-4 audit found: the run of record is committed as a function on the live-transfer lane's own pattern rather than left in a file at a path an operator names, it carries the four distinct messages whose distinctness is what gives the three refusals content, and it names itself with the same three members the reviewed contract already names the explicit run by — the ceremony's own case name, the identity the target computed, and the row it is recorded under. It deliberately omits the outpoints, which name a chain the run destroyed and would read as something a later run could resume from, and it is deliberately NOT added to the reviewed contract's dimension table, because those six dimensions are established on the explicit run and a second identity beside them would read as a second establishment of what this run did not re-establish. The row conversions are recorded by each site's own rules and are mostly refusals to convert, which is the honest result rather than a thin one. The per-case run residual in the owner-authorization census does not convert for either of two independent reasons, and both are the site's own: the run exercised a bare owner leaf rather than any of that census's authorization cases over a finalized transfer, and even a run that had exercised one could not clear the residual there, because the census refuses a case with an empty residual set and the type deliberately has no field an observed verdict could occupy — a tension between that member's own promise that a later run clears it and the validator's refusal to let it go, reported and not repaired, since redesigning another package's census on a reading is the move this workspace refuses. The carried infrastructure residual set does not convert, both its members being unrelated. The live-transfer pipeline's not-submitted standing does not convert, its own words scoping it to that pipeline's funding step, which this ceremony does not use. The paired-materialization condition does not convert, its site having already declined to be moved on a reading. The link obligations and the least ABI obligation do not convert. The explicit lane's non-claim that it establishes nothing about the proof-bearing lane stays exactly true of that ceremony and is not edited: what this wave adds is the sibling record, on the same retire-by-scope-and-not-by-wish pattern the private construction non-claims already set. Two sites are reported as bearing on the run without being converted by it, each with the sentence that stops the conversion: the ABI's confidential-field-form obligation asks for a prefix comparison exercised on the target, and while both admitted value-commitment prefixes were inside the message the target accepted a signature over — recorded in the artifact as observed prefixes rather than argued — an agreement is not the comparison that site asks for; and the resource lane's unserialized-proof non-claim has real proofs to point at now, but its own words assign the closing to the confidential-funding concept and this pipeline still serializes none. Verified entirely on the server per the no-host-compute ruling, every step timed, with the formatter of record run there and its patch applied here rather than compiled on the host |
