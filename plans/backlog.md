...
| `T5-028` | DOING | Consensus-exclusion labeling, the register and its citations: every consensus feature this workspace has elected not to support mints a label in a committed reference register, the code row that enforces the exclusion cites that label, and each section records the revision surface a future revision of the exclusion would have to touch. In flight |
