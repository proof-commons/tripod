...
| `T5-038` | OPEN | Shared abstract-program analysis, first slice: two private wrappers measure program prefixes by validating each prefix of a program and reading the deepest stack any of them reaches — one in the conformance package, one in the vectors package — and both wrap the same abstract evaluator in tapscript. This slice adds a public analysis API beside that evaluator and side-by-side tests demonstrating the public result equals each private wrapper's, with no consumer deleted and no canonical output changed; the wrappers are retired and the implementation optimized only in later slices, gated on this equivalence evidence |
