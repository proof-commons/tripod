...
| `T5-035` | OPEN | Phase-5 handoff, sponsor envelope signing, the first honest wiring step: one integration test in the live-transfer evidence lane that finalizes an explicit sponsored control, sends its exact sponsor request to the adapter, replays the returned witness through the sponsor capability, and verifies byte binding. The residual SponsorEnvelopeSignerAbsent is expected to STAY through this step and the row will record the verdict against that residual's own clearing rule rather than against the capability existing. The handoff card's stale sighash clause is re-pointed in the same work |
