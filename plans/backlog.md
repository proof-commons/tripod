...
| `T11-034` | DONE | The Wave-4 constructor core landed through `0.6.174-dev`: new `packages/tapscript/src/state_constructor.rs` (1,075 lines) and `packages/tapscript/src/tests/state_constructor_tests.rs` (903 lines, 36 tests), the manifest's production `realization`, `sha2` and `thiserror` dependencies, exports, test registration and the Meson census changed six files by +2,001/−26. The closed sum has eighteen refusals; only `TweakAboveGroupOrder`, `TweakedPointIsIdentity` and `CanonicalBranchSideNotSatisfied` are retryable. The `0..4096` nonce search selected zero with none refused, the NUMS key is recomputed on admission, control recipes enforce the 128-node limit, each transaction side has six field commitments, and seven typed reference declarations feed a component census that refuses cycles. Tapscript unit tests moved from 444 to 480; under a load average near 50, the lane's final gate round ran 196 s, and the gate of record ran 52 lanes green in 1168 s. The landing rests on rulings 22–26 and 28–30 at (`rule:phase6:wave4-rulings`) and discharges the constructor-core portions of deliverables 1–10 and the abstract boundary of all five exits. It made the oracle cross-checks in `T11-035`, the linker graph completion in `T11-036` and the anchor in `T11-037` dispatchable. |
| `T11-035` | TODO | Oracle cross-checks; its dependency `T11-034` landed through `0.6.174-dev`. Extend `packages/tapscript/src/tests/abstract_oracle_tests.rs` with the production metadata leaf under empty, true, false, arbitrary and limit-edge initial stacks; add `realization` as a test-only dependency of `packages/target-elements-conformance/Cargo.toml`; and extend `packages/target-elements-conformance/src/tests/constructor_tests.rs` to compare production STATE leaf, branch, tweak, output, parity and control results with the prototype's independent arithmetic for fixed and property-generated canonical bytes, recompute the NUMS derivation, and keep byte, root and hidden-content claims distinct. This is 2–6 production/configuration / 260–380 test lines, rests on rulings 23, 26, 29, 30 and 32 at (`rule:phase6:wave4-rulings`), and discharges deliverable 11 and exits 1, 2, 4 and 5 without promoting the prototype schema. Gate: tapscript and target-elements-conformance package tests, then the full gate of record. |
...
| `T11-040` | BLOCKED | Closure and handoff; blocked on `T11-033` through `T11-039`. Sweep `plans/backlog.md`, `plans/phases/06-state-and-maturity.md` and `plans/guides/guide_fourteen_conceptual_register.md` to record the landed rows and their tags, measurements and refusals, adopt `R-1 NONCE` and `R-3 ANCHOR` on the evidence pointers of `T11-034`, `T11-035` and `T11-037`, and retain the native rows for Wave 8 and the Wave 6, 7, 8 and 9 obligations. This is 45–80 documentation / 0 test lines, rests on rulings 25, 27–28 and 30–33 at (`rule:phase6:wave4-rulings`), and keeps each earlier row's commit subject distinct because the guide's subject belongs to `T11-034` while cross-package graph, provenance, evidence plumbing and records have distinct owners. Gate: the plan, label and census checks, then the full gate of record. |
