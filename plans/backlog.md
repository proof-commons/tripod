...
| `T5-025` | DOING | Owner-sighash parallel work, Wave 2, the signing-input census and an independent message construction: the census lands as a typed value carrying every field the accepted option-B result names, constructible only from a proof-finalized candidate; an independently written message construction consumes it, with every expectation taken from the Wave-1 source review rather than from the code path it checks; the profile's constants are asserted as constants; the typed refusals land closed; the internal-key dimension is re-typed onto the spent outputs per the owner ruling, with the review's composition argument travelling with it; and the proof-fields coverage remap lands or stops honestly. No digest is asserted as authoritative, no blocker moves, and both residuals stand |
