...
| `T5-034` | OPEN | Confidential-funding execution guide, Wave 5, restarting the Guide-13 evidence: the mandatory restart order runs in order against a real node — one accepted sponsorless private one-to-one control before any negative case, both predecessor commitment parities in complete accepted successors, target CT conservation against a balance-valid control, then the wrong-blinder, missing-rangeproof and malformed-rangeproof mutations each attributed to its own layer, the remaining positive private shapes only where each accepts, and the disclosure-minimality pairs last — with the sponsor cases NOT RUN, stopped typed on the signer dependency this guide does not close. Rows move on observed acceptance and on nothing else. Open while the runs are taken

Current blockers are: the two carried residuals of the live evidence set, `SponsorEnvelopeSignerAbsent` and `PredecessorConstructorAbsent`. The sighash blockers are cleared — the profile is established over the six-member required set and both lanes carry an observed acceptance — and the confidential predecessor is funded, so the remaining work is the signing handoff and the evidence restart of the confidential-funding execution guide.
