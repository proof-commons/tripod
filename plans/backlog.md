...
| `T5-040` | OPEN | Consensus shape-possibility register: census every blinded transaction shape the confidential live lane can take, typed per shape with its consensus verdict and its first-party status, and file a named removal path for every shape consensus admits and this workspace's own conventions refuse |
