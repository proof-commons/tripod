...
| `T5-020` | DOING | Intermediate confidential-funding guide, Wave 0, elaborating custody and reproducibility: the four preflight findings of the guide's own repository reading are reproduced by running code at the branch tree and each is given a typed disposition, the three concept rulings and the three accepted charter decisions are carried forward with the guide's fixture elaborations checked against them, the dependency boundary the guide records is re-read from the three manifests rather than assumed, and nothing whatever is implemented — the only additions are the reproduction tests themselves, marked as preflight reproductions in their own documentation. In progress |
