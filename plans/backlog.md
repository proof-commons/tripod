...
| `T11-113` | IN PROGRESS | The Wave-10 opening record: rulings 80 through 90 at (`rule:phase6:wave10-rulings`), findings 154 onward at (`sec:phase6:wave10-findings`), these six rows and the register's Wave-10 paragraph, with figures recounted and citations relocated in the opening tree. Gate: the documentation and label gate over the three edited plan files, with both minted card labels resolving. Closed by the Wave-10 closure record under `T11-118`. |
| `T11-114` | TODO | The typed edge sequence in new `packages/vectors/src/maturity_history.rs`, with `packages/vectors/src/lib.rs` and `packages/vectors/meson.build`: `StateRootEdge` carries the two outpoints, `OperationId` and a projected `MaturityTransitionCertificate`; `validate_state_root_history` applies §17.1's six clauses and `state_bundle_continuity`, returning the advanced cursor only after every edge validates, while `MaturityRootHistoryRefusal` retains the clause, position and operands. Eighteen tests comprise one accepted-archive edge, sixteen one-axis §16.8 refusals and one cursor-return test; the Meson census gains one entry and no existing test count moves. DONE when each clause and fault refuses on its own axis, the invalid intermediate edge refuses despite final cursor equality, exactly one valid edge advances the cursor, and §17.3 calls the existing linker rule. This establishes edge ordering over supplied bytes, with no report, row standing, census change or chain observation. Depends on `T11-113`. |
| `T11-115` | TODO | The checkpoint and registry in `packages/vectors/src/maturity_history.rs`, `packages/vectors/src/maturity_evidence.rs` and `packages/vectors/src/maturity_corpus.rs`: `MaturityRootCheckpoint` and `MaturityRootCheckpointRefusal` bind §17.2's ten facts, eight from admitted or public bytes and two with `MaturityPremiseProvenance`; modelled rewind and reprojection stale a removed or changed transaction; a crate-private `MaturityOutstandingMutationRegistry` constructor derives all sixteen §16.8 mutations and its `census()` reads its content. Five test cases cover the binding, mismatched archive, stale reprojection, exact-row registry and both standing directions; evidence tests move 19→21, history tests 18→21, and corpus gains one premise test. Registry mutations move 0→16, `MaturityRegistryStanding` variants 1→2 and the retired outstanding-reason variants 1→0. DONE when all ten facts distinguish checked bindings from premises, the sixteen entries are set-equal to the matrix rows, the public registry constructor remains absent, stale evidence refuses reuse, and no evidence-plan census bucket moves. This establishes neither chain freshness nor row standing. Depends on `T11-114`. |
| `T11-116` | TODO | The public handoff in new `packages/vectors/src/maturity_recovery.rs` and new `packages/vectors/tests/maturity_recovery.rs`, with `packages/vectors/src/lib.rs` and `packages/vectors/meson.build`: `PublicAnnouncementHandoff` carries §14.7's public inputs; `recover_public_successor` consumes that value alone and returns `RecoveredSuccessor` or `MaturityRecoveryRefusal`; the integration binary imports neither linker bundle nor planner. Thirteen unit tests cover nine §16.13 mutations and four type-level construction proofs; two integration tests cover accepted-archive output-0 equality and the complete import list; the Meson census gains two entries and no existing test count moves. DONE when the handoff contains exactly the public inputs, the recovery function has no second argument, every mutation refuses independently, all four private-dependency exclusions are asserted over the type, the import list is pinned, and the accepted archive's reconstructed program equals output 0. This establishes reconstructibility for that transaction, with no report, row standing, freshness, origin or relayability claim. Depends on `T11-115`. |
| `T11-117` | TODO | The two validated reports in new `packages/vectors/src/maturity_history_report.rs` and new `packages/vectors/src/maturity_recovery_report.rs`, with `packages/vectors/src/matrix.rs`, `packages/vectors/src/maturity_safety.rs`, `packages/vectors/src/maturity_evidence.rs`, `packages/vectors/src/maturity_negative_half.rs`, `packages/vectors/tests/maturity_recovery.rs`, `packages/vectors/src/lib.rs` and `packages/vectors/meson.build`: `MaturityRootHistoryReport` and `MaturityPublicRecoveryReport` have separate assemble-and-validate pairs, roles, residuals, censuses, recomputed items and canonical bytes; `EvidenceBoundary` gains `RootHistoryReportRejection` and `PublicRecoveryReportRejection`, and `classify` enters the matching standing only for a validated report. The sixteen §16.8 and fourteen public-recovery rows take those boundaries; the two displaced typed reasons leave `MaturityRowBoundary`. Counts move: boundaries 13→15, row-boundary variants 7→5, typed non-answers 65→35, answered 41→71, root-history observed 0→16, public-recovery observed 0→14, and the partition six→eight buckets summing to 206; `STILL_REQUIRED` stays 42, evidence tests move 21→22, safety tests stay 20, and Meson gains two entries. DONE when both reports validate and render three ordered residuals over the accepted archive, reject changed claims at named items, keep their role bytes separate, route exactly the thirty named rows, leave rows outstanding on failed validation, recompute the 206-row partition and register set equality, and remove both retired reasons everywhere. The ten acceptance-awaiting positive rows retain their separate question; neither report establishes freshness, chain observation, archive origin, artifact promotion or the other's evidence. Depends on `T11-114` through `T11-116`. |
| `T11-118` | TODO | The Wave-10 closure or precisely partial stopping record: recount the landed tests, boundaries, standings, census and register; dispose of each bite on its gate evidence; state the four exits against the study's expected MET, MET, CONDITIONALLY MET and split MET/NOT MET statuses, including §23's stronger conditional edge claim; record only the `G14C-02`, `G14C-11`, `G14C-12` and `G14C-13` clauses actually advanced, the evidence-based `R-5 RECOVERY` disposition, and the residuals for chain observation, cursor freshness, archive origin, declared checkpoint premises, acceptance-awaiting rows, fee-floor relayability, resources and artifact promotion. DONE when the evidenced disposition closes `T11-113` without treating a modelled branch or public-byte reconstruction as a chain or origin proof. Gate: the closure's documentation, label and census checks over the landed evidence. Depends on `T11-117`. |
