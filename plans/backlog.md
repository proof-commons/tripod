...
| `T5-030` | DOING | ADR-019 adoption-parameter split, per-row labels for the seven fixings: the single parameters-table entry becomes seven labeled entries, one per adoption parameter, so an amendment cites the parameter it amends rather than the whole table; the split is recorded as ADR-019's own labeled amendment entry, and ADR-023's two-row citation moves to the two precise entry citations in the same commit. |
