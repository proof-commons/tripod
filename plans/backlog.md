...
| `T5-046` | ACTIVE | Explicit-runs wave, ceremony built and not yet run: a shape-parameterized explicit-lane ceremony now exists at live_explicit_shapes, the sibling of the private lane's multi-shape ceremony and built on the explicit spine the owner-observation lane established — issue a disposable asset, relink against it, fund each published owner's explicit constructor, finalize over the funded coins, census, sign per input with whichever owner holds it, submit, and read the mined bytes back. Thirteen of the sixteen rows have a shape and a thin ignored test each; the shape varies only in coin counts, destination owners, destination values and the order the receipts are offered, so two runs that differ are runs whose shapes differ. Nothing is answered yet: no row moves until a node has accepted a transaction of that row's own shape. Drive the §15.1 positive explicit table, answered in ZERO of its sixteen rows, onto observed acceptances of each row's own shape at the pinned node through the same NativeRunObserved standing the private rows use, then answer the explicit negative rows that become attributable once their positive control exists — a refusal counting only when the unmutated form is accepted and the mutated one is refused naming the row's own class. Rows move on observed results alone; a row that cannot be answered honestly gets a typed stop with a filed path rather than a narrated skip |
