...
| `T11-016` | TODO | B4 is ready because `T11-015` (B3) landed: add the plan boundary in new `packages/compiler/src/maturity_announcement_plan.rs`, a bespoke validated container reusing the shared generic row vocabulary of `packages/compiler/src/operation_plan.rs`, with fields and helpers for both representation modes, every factor agreeing, a direct declaration census, and no public unchecked construction; add the exports, error wiring, and census entries. Estimate: 1,200–1,600 lines. |
...
| `T11-021` | TODO | Architecture: name the announcement lead bounds. Add two entries to the public `BoundId` census in `packages/architecture/src/ids.rs` for the announcement lead minimum and maximum; declare them beside the existing bounds in the announce-maturity architecture row in `packages/architecture/src/spec.rs`; recompute and re-pin the architecture semantic hash under ADR-021 recipe permanence (`[ADR021-rule:identity:recipe-permanence]`) with the register record in `plans/registers/identities.md`; resolve the realization's `AnnouncementLead` fact keys to those bounds; and prove by model conformance test that `min_maturity_lead` and `max_maturity_lead` in `packages/model/src/constants.rs` agree with the architecture values. This bite must land before `T11-017` (B5), which gains the altered-lead-bounds corruption check. Estimate: 150–250 production and 100–200 test lines, plus the register record. |
