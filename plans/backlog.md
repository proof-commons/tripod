...
| `T5-036` | TODO | Internal-key unspendability probe, phase A: one key-path spend attempt against an already-funded explicit-constructor output, through the existing generic submission wire unchanged, recording the exact bytes submitted, the one-item witness shape, the funded outpoint and its program binding, the target's provenance, and the refusal verbatim. Chartered from the Phase-5 handoff row of the same name. Phase B, the typed carrier and the wire correction, is out of scope here. A refusal discharges only that the attempt was observed and refused; it establishes nothing about who knows the discrete logarithm, so the residual assumption stands regardless of what this probe observes |
