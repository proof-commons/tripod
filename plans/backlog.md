...
| `T4-008` | PARTIAL | Real target execution (Wave 11). The whole path is built and a live node ran it: the section 16.2 edge taken, `CompactAshOperationPlanner` mapping the thirteen obligations onto operation steps, materialization taking its outpoints from funding answers rather than a fixture ordinal, the adapter performing funding and submission for real, and `CoverageObservation` gaining its observed arm whose discharge requires acceptance and a matched projection both. Two findings block the deliverables and one defect was fixed. The fixture bundle is unfundable on any real chain: all twelve linked leaves substitute the chosen `CLOSED_ASSET` constant, so the pin is a function of it, and an Elements asset identifier is derived from the issuing outpoint rather than chosen — `bundle::ceremony_bundle` therefore links a second bundle at the asset a ceremony issues and derives its pin from its own tree. The ceremony's first step then stopped on a target refusal that reproduces with no first-party code at all (`T4-012`). Fixed on the way: `ExecutorCapability` had no `FreshProcessLifecycle` variant while the adapter has advertised one since Guide 11, so every wallet-enabled handshake was unreadable. Remaining: the ceremony past issuance, the nine submissions, projection comparison against a real acceptance, coverage discharge, and the five sponsored cases |
| `T4-012` | BLOCKER | The funding ceremony's issuance step is refused by the target, and the refusal reproduces with no first-party code. Recipe, entirely node tooling on `elementsregtest` at `v28.99.0-b7fc5d080a7e`: create a wallet, take one explicit policy-asset coin well below the money bound, `createrawtransaction` spending it into a change output and a fee output that balance it exactly, `rawissueasset` that base with `asset_amount` 210000 and `blind` false, then `testmempoolaccept`. The node answers `bad-txns-in-ne-out` — before and after `signrawtransactionwithwallet` — on a transaction its own `decoderawtransaction` reports as balanced in both assets: issuance amount in equals asset out, and policy in equals policy out plus fee. Two hypotheses were tested and disproved: rebuilding the outputs is not the cause, because the node's own unmodified `rawissueasset` product is refused identically; and a missing witness section is not the cause, because a flag-set transaction with empty witness entries is refused as a superfluous witness record. A third is untested and most likely — that the working coin in the reproduction is a blinded wallet output whose value consensus cannot add explicitly — but it does not carry to the adapter, whose coin is the chain's explicit anyone-can-spend free coin. Next step is to bisect on explicitness of the funding coin, then file the surviving case in the Elements gripe register. Related and separate: the money bound is checked against the total across all assets, not per asset, which is why the ceremony must park the free coin before issuing |
...
Phase-4 work:
    ...
    ran it. It waits on the issuance refusal recorded as T4-012, which
    stops the funding ceremony at its first step and therefore stops
    every submission behind it
```
