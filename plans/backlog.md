...
| `T5-021` | DOING | Intermediate confidential-funding guide, Wave 1, revising the funding wire: the fifth operation subject arm, its step kind and capability, the request and response records of the guide's wire section, the protocol revision moved to five in the harness and the reviewed native adapter together, the confidential funding advertisement held consistent with the capability, the reproducibility contract placed in the reviewed target package with that package's contract note widened in the same change, the complete typed wire refusal vocabulary, readback binding that takes no output fact from a request echo, and mock-executor and protocol contract cases for every arm and every mismatch. Nothing reaches a target and no blocker moves |
...
| `DI-F07` | DONE | User ruling: the retirement of the four adopted drafts into ADR-019 through ADR-022 is not complete. Each repository amendment must be individually labeled and individually listed in the record's Amendments section — one entry per amendment, each carrying its own label and stating exactly what it changes in the normative text — and no amendment may be merged into the adopted body transparently. The closing work audits each of the four records against its adopted draft in Git history, enumerates every divergence between the present body and the draft bytes, and for each divergence either restores the draft text and promotes the change to its own labeled amendment entry or, where it already stands in the Amendments section as grouped prose, restructures it so every amendment is a separately labeled, separately listed entry. Closed. The audit diffed each record's adopted body against the draft bytes of record — the drafts as they stood at the parent of the retirement commit, after the third-edition swap and the interchange text's fourth-edition refresh — and found all four bodies byte-identical to their drafts save one uniform divergence: folding demoted every heading by exactly one level, nine lines in ADR-019, twenty-four in ADR-020, nine in ADR-021, seven in ADR-022, and that change stood in no Amendments section. Regressing it was refused, a record carrying exactly one title, which is the record's own, so each of the four now lists the demotion as its own labeled heading-depth entry stating what moved and what did not. Two further listing defects were repaired. ADR-019's implementation-standing entry was the one entry across the four records carrying no label at all, and takes one. ADR-020's registry-sense-and-attestation entry carried three amendments as grouped prose — the sense an unqualified registry takes, the standing of the edition's statuses against the located evidence, and the designation of the generated companion register — and becomes three separately labeled entries, the existing label kept on the middle one, which nothing outside the record cites. Seven mints in all: six new entries plus the label the unlabeled entry took. The Amendments sections now list four entries in ADR-019, nine in ADR-020, six in ADR-021, and five in ADR-022, twenty-four entries with twenty-four labels and none merged into a body. The seven adoption-parameter rows of ADR-019 were left as one table entry deliberately, because ADR-023 amends two of its rows by citing the table and reading it row by row, so dissolving it would break that record's own amendment; the same reading keeps the other data tables whole, and the boundary drawn is that a table is one amendment whose content is tabular while separate prose statements are separate amendments. The ADR writing rules now carry the discipline for every future record adopting an externally authored text. Verified on the shared instance under the no-host-compute ruling: the worker lane on the labels package green at exit 0, 208 tests, 6.7 s; check-plans green at exit 0, 0.43 s; the meson lint suite green, 9 of 9 lanes, 8.8 s; and the documentation and generated suites green, 4 of 4 lanes, so labels-check resolves the new mints and check-generated confirms no generated register needed regeneration. |
