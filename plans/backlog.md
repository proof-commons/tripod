...
| `T5-045` | OPEN | Removals-completion wave: finish the two structural removals the shape-census wave landed at the registry and typed-stopped one layer short of a run, on the rule that a removal is complete only when the shape RUNS. The private merge is no longer refused by the two-output floor; it is refused by the ZERO BLINDER its only available predecessor forces, the dual-parity predecessor's two output blinders being ordered additive inverses, so the solved sole-balancing blinder cancels and DegenerateBalancingScalar fires — the filed path is a CHAINED NON-CANCELING PRECURSOR, a predecessor whose spendable coins carry blinders that do not sum to zero. The one-to-one-with-fee shape is refused at FeeRoleNotProjectable, the materializer's role vocabulary, its per-output stage and the executor adapter not projecting the fee role, and mapping fee onto balancing would produce a blinded fee output, which is not a fee. This row carries both runs or their typed stops, the latent required_prefixes arity panic in the conformance confidential record, and the census, register and erratum updates each acceptance earns |
