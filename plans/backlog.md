...
| `T11-016` | TODO | B4 is ready because `T11-021` landed: add the plan boundary in new `packages/compiler/src/maturity_announcement_plan.rs`, a bespoke validated container reusing the shared generic row vocabulary of `packages/compiler/src/operation_plan.rs`, with fields and helpers for both representation modes, every factor agreeing, a direct declaration census, and no public unchecked construction; add the exports, error wiring, and census entries. Estimate: 1,200–1,600 lines. |
| `T11-017` | BLOCKED | B5 is blocked on `T11-016` (B4) only: add validators and corruptions across `packages/compiler/src/maturity_announcement_plan.rs`, `packages/compiler/src/maturity_announcement_requirements.rs`, and new `packages/compiler/src/tests/maturity_announcement_plan_tests.rs`. Require full source/projection/case/coverage comparison, cross-level evidence closure, the corruption list — omission, duplication, extra identity, body, case mode, activity, coherent relation/dependency omission, and altered lead bounds after `T11-021` resolves their fact keys to architecture bounds — plus the supplemental obligation/dependency census and sponsor traversal. Estimate: 250–400 production and 500–750 test lines. |
