...
| `T5-039` | IN PROGRESS | Confidential-funding execution guide, Wave 7, clearing the step-five stop by building the remaining positive private shapes as multi-output and multi-input fixtures and running each against the pinned node, moving its §11.2 row only on an observed acceptance of its own shape; the one-output shapes (private-merge, and the completeness one-in/one-out and fee-only cases) are typed-stopped honestly against the registry's two-output floor rather than forced, with the merge-predicate conflict filed as a guide erratum; deterministic-public-fixture-openings assessed against the byte-identity contract it actually asks for; step-seven reachability assessed against the guide's own ordering text; new machine-checked wave_seven_closeout on the wave_six precedent, the row delta quantified, and the Phase-5 exit reachability reported. |

Current blockers are: the two carried residuals of the live evidence set, `SponsorEnvelopeSignerAbsent` and `PredecessorConstructorAbsent`. The sighash blockers are cleared, the confidential predecessor is funded, the signing handoff is closed, and the evidence restart has taken its first four steps against a real node — three positive private matrix rows are answered by observed acceptances, target CT conservation among them, and the three proof-negatives are observed at the mempool boundary and attributed by field. The restart order stops typed at step five, whose remaining positive shapes need multi-output and multi-input fixtures nobody has built yet, with private-merge structurally unconstructible; steps six and seven follow the sponsor dependency this guide does not close. The remaining work is the step-five shapes and the step-seven disclosure-minimality pairs, both behind fixtures and a dependency a following wave owns.
