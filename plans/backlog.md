...
| `T11-111` | TODO | Accepted-byte row classification in `packages/vectors/src/maturity_evidence.rs`: the standing classifier enters `ConstructorContinuityObserved` for rows observed by the accepted archive's validated continuity report and `NativeAcceptanceObserved { identity }` for the sponsorless positive row on the accepted target identity, only from present constructor material whose acceptance is `Established` under the variable schedule with matching readback identity. The standing records the disclosed zero relay and block fee floors. Recount `is_answered`, the census and register set equality without changing boundary declarations. Ruling 73 at (`rule:phase6:wave9-rulings`) reserves the accepted-byte standing, and the closure at (`sec:phase6:wave9-findings`) states its evidence boundary. Gate: both standings arise only from established acceptance; refused or node-free projections enter neither; the recomputed partition and register equality hold, with every affected census or report pin moved wherever it lives. Depends on `T11-099`; scheduled beyond the wave. |
