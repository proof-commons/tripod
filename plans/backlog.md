...
| `T5-037` | IN PROGRESS | Confidential-funding follow-up evidence wave, restarting the order from its third step under the orchestrator's interlock ruling that Wave 5's step-three stop was unwarranted: re-run steps one and two on fresh disposable chains, record target CT conservation citing both an accepted balance-valid control and one wrong-blinder mutant's balance-layer refusal as its non-conserving half, run the three proof-negatives attributed by mutated field at the mempool boundary, reach the positive private shapes step five actually accepts and the disclosure-minimality pairs where both sides accept, and machine-check a new closeout. The step-three disclosure names in words that its refused half is the same observed run step four records as WrongBlinder so two Accepted ledger entries cannot be double-counted. Repo repairs: the CtConservation role doc records its "refused a non-conserving one" clause as a repo-authored strengthening of §10.5 rather than a guide requirement, and the new closeout names Wave 5's step-three mistyping (it cited NoAcceptingControlExists against a ledger holding an accepted control) as corrected. One guide-side erratum filed for step four's undischargeable "each attributed to its own layer" — the target emits one identical reject string for all three mutations. |
