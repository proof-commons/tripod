...
| `T5-029` | DOING | Owner-sighash parallel work, the issuance re-typing and the completed verdict: the post-verdict owner ruling re-types the issuance dimension from required to refused, so the profile's required set becomes six, the capability's unestablished table records the dimension as one the selected profile refuses, the disposition recomputes to established over a six-member required set every member of which is reviewed, and the profile residual clears in all four vocabularies on that recomputation rather than on any declaration. The consensus-exclusion register's issuance-dimension section moves from owed rows to as-landed rows. Routed with it are the three coverage findings that section recorded in passing, and the pre-existing documentation break inherited from the branch point |
