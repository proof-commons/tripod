//! Shared CLI scaffolding for workspace command-line tools (ADR-010).
//!
...
//! # Output contract
//!
//! Stdout carries **result data only**: at most one JSON object plus a
//! trailing newline, and nothing at all for a side-effect command.
//! Every diagnostic, status, help text, version string, usage error,
//! and panic report goes to stderr as JSON. There is no human-prose
//! mode and no partially structured mode.
//!
//! Result data is refused when stdout is a terminal, because a JSON
//! result is machine input; the refusal is exit class 2 with a
//! [`ControlPlaneRecordKind::TtyRefusal`] record on stderr.
//!
//! # Boundary
//!
//! This crate never renders raw argv, environment values, or child
//! process output. Usage-error records carry the clap error *class*
//! and no argument text; panic payloads are omitted until
//! [`set_panic_payload_reporting_enabled`] is called with the parsed
//! `--debug` value; arbitrary `Display` errors routed through the
//! runners pass [`redact_text`] first. It performs no argument
//! parsing of its own beyond delegating to `clap`, and it resolves no
//! repository path from its compiled location — subjects arrive by
//! argument (ADR-014).
//!
//! # Map
//!
//! - **Exit classes** — [`CommandExit`] (0 success, 1 failure,
//!   2 usage) and [`exit_with`].
//! - **Control-plane records** — [`ControlPlaneRecord`] with
//!   [`ControlPlaneRecordKind`] / [`ControlPlaneFields`], written by
//!   [`emit_control_plane_record`]; [`CliExit`] pairs a record with
//!   its exit class.
//! - **Argument parsing** — [`parse_args_from`], [`parse_args_or_exit`],
//!   and the flattenable [`BaseArgs`] / [`CheckOutputArgs`].
//! - **Redaction** — [`redact_text`], [`redact_url`],
//!   [`redact_field_value`], [`is_sensitive_field_name`], [`REDACTED`].
//! - **Process scaffolding** — [`install_json_panic_hook`],
//!   [`set_panic_payload_reporting_enabled`],
//!   [`init_json_tracing_with_debug`], [`tracing_filter`],
//!   [`refuse_if_stdout_is_tty`], [`emit`].
//! - **Runners** — [`run_stdout_json_command`], [`run_no_stdout_command`],
//!   [`run_no_stdout_command_async`], [`run_check_command`].
//! - **Output roles** — [`destination_identity`],
//!   [`ensure_distinct_outputs`], [`AliasedOutputs`], [`touch_stamp`],
//!   [`finish_check_command`].
//! - **Publication** — [`PublicationMode`], [`PublicationAsset`],
//!   [`publish_batch`], [`PublicationChange`], [`PublicationResult`],
//!   [`BatchPublicationError`], [`set_publication_mode`].
//!
//! # Example
//!
//! A control-plane record is a value first and a side effect second, so
//! its JSON shape can be asserted without touching stderr:
//!
//! ```
...
//! // The offending argument text is deliberately absent.
//! assert_eq!(record.message, "invalid command-line arguments");
//! assert_eq!(CommandExit::Usage.code(), 2);
//! ```
//!
//! See `README.md` in this package for the full command skeleton,
//! the stderr transcript shapes, and the checker two-mode contract.
