...
fn indexed_burn_world() -> (World, ReferenceIndexer) {
    ...
    let indexer = ReferenceIndexer::from_history(&burned.history, &chain, [0_u8; 32]).unwrap();
...
fn query_uses_indexer_bound_context() {
    ...
    let indexer = ReferenceIndexer::from_history(&world.history, &chain, [0_u8; 32]).unwrap();
