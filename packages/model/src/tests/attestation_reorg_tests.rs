...
/// A reorg is a change of chronology
/// `(´[A-postc:interface:monotonicity]´)` and `(´[RZ-sec:ledger:reorg]´)`:
/// the same immutable burn record is revalued upward or downward
/// depending on which settlement precedes it in the selected
/// context. Neither direction is privileged; only the raw
/// record is invariant.
///
