...
impl RenderPaths {
    fn new(root: &Path) -> Self {
        Self {
            ...
            stamp: root.join("attestation-stamps.ok"),
        }
    }
    ...
    fn assert_absent(&self, name: &str) {
        ...
        assert!(!self.stamp.exists(), "{name}: stamp file was created");
    }
}
...
fn run_case(root: &Path, paths: &RenderPaths, case: RenderCase) -> Output {
    ...
    if case.stamp {
        push_path_arg(&mut args, "--stamp", &paths.stamp);
    }
...
fn assert_usage_error(case: RenderCase, output: &Output) {
    assert_eq!(output.status.code(), Some(2), "{}: exit code", case.name);
    assert!(
    ...
    assert_eq!(lines.len(), 1, "{}: expected one JSON record", case.name);
    ...
    assert!(
        ...
        "{}: clap error kind must be present",
        case.name
    );
}
...
fn partial_render_argument_sets_are_usage_errors_without_side_effects() {
    let cases = [
        ...
        RenderCase {
            name: "template and stamps plus epoch",
            template: true,
            stamps_output: true,
            epoch_output: true,
            stamp: false,
        ...
        RenderCase {
            name: "template and stamps plus stamp",
            template: true,
            stamps_output: true,
            epoch_output: false,
            stamp: true,
        ...
        RenderCase {
            name: "template and epoch plus stamp",
            template: true,
            stamps_output: false,
            epoch_output: true,
            stamp: true,
    ...
    for case in cases {
        ...
        assert_usage_error(case, &output);
