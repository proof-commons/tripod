...
fn assert_fact(fact: crate::FactId, ty: crate::SemanticType, reason: crate::DisclosureReason) {
    ...
    let (graph, nodes) = crate::declassification::build_disclosure_graph([
        crate::DisclosureNode::Fact { id: fact.clone(), initial_visibility: crate::InitialVisibility::Private },
    ], []).unwrap();
    assert!(matches!(&graph[nodes[&crate::DisclosureNodeId::Fact(fact.clone())]], crate::DisclosureNode::Fact { initial_visibility: crate::InitialVisibility::Public, .. }));
...
fn state_fact(field: crate::StateField) -> crate::FactId {
    crate::FactId::StateField { operation: OperationId::AnnounceMaturity, field }
}

#[test]
fn state_field_names_are_complete_distinct_and_in_declaration_order() {
    use crate::StateField;
    assert_eq!(StateField::ALL, &[StateField::Omega, StateField::YL, StateField::YT, StateField::Q, StateField::Cycle, StateField::Maturity]);
    let names = StateField::ALL.iter().map(|field| field.name()).collect::<Vec<_>>();
...
fn state_omega_has_its_owner_type_and_public_state_seed() {
    assert_fact(state_fact(crate::StateField::Omega), crate::SemanticType::Amount, crate::DisclosureReason::PublicState);
}

#[test]
fn state_yl_has_its_owner_type_and_public_state_seed() {
    assert_fact(state_fact(crate::StateField::YL), crate::SemanticType::Amount, crate::DisclosureReason::PublicState);
}

#[test]
fn state_yt_has_its_owner_type_and_public_state_seed() {
    assert_fact(state_fact(crate::StateField::YT), crate::SemanticType::Amount, crate::DisclosureReason::PublicState);
}

#[test]
fn state_q_has_its_owner_type_and_public_state_seed() {
    assert_fact(state_fact(crate::StateField::Q), crate::SemanticType::Amount, crate::DisclosureReason::PublicState);
}

#[test]
fn state_cycle_has_its_owner_type_and_public_state_seed() {
    assert_fact(state_fact(crate::StateField::Cycle), crate::SemanticType::Cycle, crate::DisclosureReason::PublicState);
}

#[test]
fn state_maturity_has_its_owner_type_and_public_state_seed() {
    assert_fact(state_fact(crate::StateField::Maturity), crate::SemanticType::Maturity, crate::DisclosureReason::PublicState);
}

#[test]
fn requested_cycle_has_its_owner_type_and_public_request_seed() {
    assert_fact(crate::FactId::RequestedAnnouncementCycle { operation: OperationId::AnnounceMaturity }, crate::SemanticType::Cycle, crate::DisclosureReason::PublicRequest);
}

#[test]
fn minimum_lead_has_its_owner_type_and_public_request_seed() {
    assert_fact(crate::FactId::AnnouncementLead { operation: OperationId::AnnounceMaturity, bound: crate::AnnouncementLeadBound::Minimum }, crate::SemanticType::Cycle, crate::DisclosureReason::PublicRequest);
}

#[test]
fn maximum_lead_has_its_owner_type_and_public_request_seed() {
    assert_fact(crate::FactId::AnnouncementLead { operation: OperationId::AnnounceMaturity, bound: crate::AnnouncementLeadBound::Maximum }, crate::SemanticType::Cycle, crate::DisclosureReason::PublicRequest);
}
...
fn reject_numeric(field: crate::StateField, sum: bool) {
    ...
    let id = ExprId::relation(RelationId::new(OperationId::AnnounceMaturity, RelationKind::ExpressionPredicate, RelationSubject::Operation), ExpressionRole::Predicate);
    let node = if sum { ExpressionNode::CheckedSum { ty, terms: vec![input.clone()] } } else { ExpressionNode::LessOrEqual { left: input.clone(), right: input.clone() } };
    let error = crate::build_expression_graph([
        ExpressionDeclaration { id: input, ty, node: ExpressionNode::Fact(fact) },
        ExpressionDeclaration { id, ty: if sum { ty } else { SemanticType::Bool }, node },
    ]).unwrap_err();
    assert_eq!(error, if sum { RealizationError::InvalidSumType(ty) } else { RealizationError::InvalidOrderedType(ty) });
}
...
fn cycle_values_have_only_the_cycle_accessor() {
    ...
    facts.insert(state_fact(crate::StateField::Cycle), value).unwrap();
}

#[test]
fn maturity_values_have_only_the_maturity_accessor() {
    for maturity in [crate::Maturity::Unannounced, crate::Maturity::Announced { cycle: crate::Cycle::new(9) }, crate::Maturity::Complete] {
        ...
        assert_eq!(value.as_owner_set(), None);
        let mut facts = crate::FactValues::default();
        facts.insert(state_fact(crate::StateField::Maturity), value).unwrap();
    }
}

#[test]
fn state_family_amount_remains_distinct_from_metadata() {
    let family = crate::FactId::FamilyAmount { operation: OperationId::AnnounceMaturity, side: TransactionSide::Input, object: ObjectId::State };
