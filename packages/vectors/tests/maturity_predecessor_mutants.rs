//! Native predecessor-constructor mutant offers and their scripted capture form.
...
use vectors::maturity_mutant_ceremony::{
    MaturityMutantCeremonyPlanner, MaturityMutantEvidence, PREDECESSOR_MUTANT_STEPS,
    declared_layer, roster,
};
...
const TABLE: MaturityMutantTable = MaturityMutantTable::PredecessorConstructor;
...
fn the_maturity_predecessor_mutants_are_refused_before_their_control_is_accepted() {
        ...
        OperationSubject::Submission(subject) => {
            ...
            if response.observed_layer == ObservedOutcomeLayer::Accepted {
                ...
                response.observed_detail = Some(
                    match step.case().step.as_str() {
                        name if name == PREDECESSOR_MUTANT_STEPS[5] => {
                            "Script failed an OP_VERIFY operation"
                        }
                        name if name == PREDECESSOR_MUTANT_STEPS[6] => "Invalid Schnorr signature",
...
fn scripted_exchange() -> (
    ...
    let responses = scripted
        ...
        .collect::<Vec<_>>();
    let directory = common::test_directory("predecessor-mutant-scripted");
...
fn shared_capture_names_preserve_the_legacy_roster() {
    let directory = common::test_directory("predecessor-mutant-capture-names");
