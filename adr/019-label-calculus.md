...
The normative text fixes kind and area as words without hyphens. This repository amends the area segment only: **an area may hyphenate words exactly as a name may**. The corpus already carries hyphenated areas, `labels-index` among them, and the concession costs nothing the calculus relies on — the colon, not the hyphen, is the segment separator.
