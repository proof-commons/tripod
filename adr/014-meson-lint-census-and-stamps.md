...
Meson computes the file census at configure time from `git ls-files` and
partitions it into role groups: documentation Markdown, plans, ADRs,
LaTeX sources, per-crate Rust sources, and committed generated
artifacts.

Only tracked files are lint subjects. An untracked scratch file is
invisible to the census, which aligns the lint gate with the clean-tree
law (`[ADR011-rule:toolchain:clean-tree]`): what CI validates is exactly
what the repository records.

## Census staleness is a hard failure · `rule:build:census-verification`

The discovery walk of (`[ADR013-rule:labels:census]`) survives as a
verifier, not a source. Each checker re-discovers its subjects on disk
and hard-fails when the argument census and reality disagree, with a
diagnostic naming the missing or extra paths and instructing a
`meson setup` rerun.

A cheap always-stale build target additionally diffs `git ls-files`
against the configure-time capture on every build.

The ADR-013 completeness guarantee is therefore preserved in a stronger
form: a newly added file cannot silently sit outside the label graph —
it can only fail the build until the census is regenerated.
...
- The true input set of every lint is visible in the generated build
  file and diffable in review.
- Adding or removing a tracked file requires a reconfigure; until then
  the census verification of (`rule:build:census-verification`) fails
  loudly. This is the same configure-time-capture trade the repository
  already accepts for the source date epoch.
...
**A committed census manifest.** A checked-in file list validated by CI
is the allowlist that (`[ADR013-rule:labels:census]`) exists to forbid,
with churn on every file addition.
...
- every checker accepts `--stamp` with touch semantics from the shared
  ...
  outputs;
- the top-level build partitions `git ls-files` into role groups and
  passes them explicitly to every checker and generator target;
- a file added, removed, or renamed without reconfiguring fails the
  census verification with a diagnostic naming the paths;
