...
(`[ADR019-judg:labels:minting]`); the checker command-line surface under
ADR-010
...
2. The carrier discovery walk of (`[ADR019-judg:labels:minting]`) survives
...
- **Always-fresh repository audits** — `census-audit`, `forbidden-text-check` and `hash-citations-check` — ask git for the complete tracked set on every build, so they are `build_always_stale` and may run each build. What they have in common is that the thing they look for arrives with a file rather than with an edit to a declared one: a file committed without a census entry, a newly committed forbidden token, a hexadecimal value in a file no input list names. Each must fail immediately rather than at the next reconfigure, and an input list is exactly what cannot see it. Each writes a compare-if-changed report, and no downstream edge depends on its retouched stamp, so running them does not cascade the incremental suites.

The no-op guarantee therefore applies to the incremental lint suites and the generators, not to the repository audits, which are intentionally unconditional.
...
concern of (`[ADR019-judg:labels:minting]`) is answered not by
...
- a second `ninja` invocation on an unchanged tree runs no incremental lint or generator command; the always-fresh repository audits (`census-audit`, `forbidden-text-check`, `hash-citations-check`) may run, but write compare-if-changed reports that cascade nothing;
