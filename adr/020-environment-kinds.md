...
The normative text parameterizes the effective registry by an acceptee A. This repository is that acceptee and the only one; it owns the local extension set X_A, the first-hand evidence and attestation status of every extension, and the generated companion register. The effective relation C_A is the normative registry relation C together with X_A.

### Extension entry · `rule:kinds:extension-entry`

The normative text permits an acceptee to add a pair only when its token and concept are distinct from every registry entry and first-hand evidence is located. This repository applies that rule to the recorded extension set below; a genre already classified by the registry is migrated rather than admitted as an extension.

### Migration record · `tab:kinds:migration`

The conflicting tokens adjudicated against the concept they name have the following completed outcomes. The first six migrations were completed on 2026-08-16; the later edition and three-part-label rounds completed the remaining migrations.
...
| `mthm` | Meta-theorem is `metathm` | Same genre, two spellings of one abbreviation | migrated to `metathm` |
| `ver` | Version and Revision are `ver`; Verification, Check, and Sanity check are `verif` | **Different genre, colliding token.** The affected attestation walkthrough anchors are verifications, not version statements | migrated to `verif` |
| `abs` | Abstract is `abst`, which Synopsis shares | Same genre, two spellings of one abbreviation: the mint is the paper's own abstract environment | migrated to `abst` |

### Recorded extension set X_A · `tab:kinds:extensions`

The pairs this repository adds to the normative registry relation, each with first-hand evidence in the corpus, are:
