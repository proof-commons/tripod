...
**Status:** Decided and adopted; normative now and unimplemented by design until an externally consumed document exists.
**Document:** This ADR is the normative fourth and forward-compatibility edition, supplied 2026-08-18, of _The Interchange Conventions_, formerly the archived adopted-source draft.
**Provenance:** Externally authored in its fourth edition; normative here by adoption rather than by authorship.
**Scope:** Every document this repository emits for consumption outside itself.
**Excludes:** First-party command-line streams and the native executor protocol, both of which ADR-010 owns.
**Does not establish:** Any namespace claim, assigned theory, identity over an interchange document, or obligation on a document that never leaves this repository.
...
ADR-010 governs first-party command-line streams: what a workspace executable writes to standard output and standard error, its JSON result encoding, shared control-plane records, diagnostics, and exit classes. Those streams are consumed inside this repository by the process that chose the invocation.

ADR-022 governs externally consumed documents: artifacts that leave this repository for a reader that did not invoke their producer. Their encoding, envelope, versioning, and acceptance follow the normative text below.

The two scopes do not overlap and neither defers to the other. A first-party stream carries no namespace label or interchange version triple and is not archived for later validation against an assigned theory. A document has the converse role. Neither record amends the other, and any future surface that appears to fall under both must be adjudicated explicitly.

### Executor protocol stop · `rem:interchange:executor-stop`

The native executor protocol in `packages/target-elements-conformance/src/executor.rs` and its mock peer remains outside this ADR. It is a line-delimited JSON pipe between two first-party processes in one repository, with no archive, third-party reader, version negotiation, or consumer decision changed by a frame digest; ADR-010 owns its shape.

The stop is revisited if the frame stream becomes archived, read by a third party, or consumed by a release as evidence. At that point it becomes an externally consumed document and must be adjudicated under this ADR. The protocol's revision counter is internal pipe versioning, not a version in the normative interchange sense (`def:interchange:versions`).
...
Adoption is normative now and builds nothing now. No registry (`sig:interchange:theory-assignment`), namespace allocation, canonical encoder, or validating decoder is built until a real externally consumed document exists. Until then the governance obligations of (`cav:interchange:governance-obligations`) bind nothing because there is no assignment to allocate, consolidate, or preserve.

The first externally consumed document must arrive with the registry, allocation, encoder, and decoder work its content requires. Compact-attestation-era artifacts are the expected first consumer class, but this ADR specifies, schedules, and requires no such artifact.
